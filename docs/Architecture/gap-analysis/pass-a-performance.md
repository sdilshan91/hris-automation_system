...
> **✅ RESOLVED 2026-08-17.** Decided AMEND, not build ([[ADR-2026-08-11-goal-ownership-stays-individual]]) — the tier is **withdrawn, not deferred**. Pass B `:93` is now `PARTIAL`. Everything below in this lead is the evidence that produced that decision; read it as history, not as open work.
...
7. ~~**G7 — FR-4 cascading has no org/department tier. MED.**~~ **✅ CLOSED 2026-08-17.** *Smallest **honest** fix: either build the owner tier (L) or **record a scope decision** that cascading is individual-only and amend FR-4/§11.9. Recommend the decision first; the UI is missing either way.* → **The recommended path was taken:** decision first ([[ADR-2026-08-11-goal-ownership-stays-individual]]), then FR-4 + §11.9 amended, then the rating downgraded and the doc tree propagated.
...
- ~~**type:** doc-drift · **severity:** MED · **where:** tech doc `:850` vs `Goal.cs:18,48` · **what:** §11.9 promises org→department→individual cascading; the model supports only individual→individual. · **suggested-action:** **decision gate** — build the owner tier or amend the doc; either way downgrade Pass B's rating and record an ADR.~~ **✅ CLOSED 2026-08-17.** Decided AMEND ([[ADR-2026-08-11-goal-ownership-stays-individual]]); tech doc `:850` and US-PRF-001 FR-4 amended 2026-08-11; Pass B `:93` downgraded COVERED→PARTIAL and the remaining 6 doc-tree contradictions propagated 2026-08-17. **All three prescribed actions are now actually done** — the ADR and the two headline amendments landed on 2026-08-11, but the downgrade and the propagation did not, and the completion plan claimed otherwise for six days.
