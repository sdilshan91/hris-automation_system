...
| **GAP-007** | **276 executable `IgnoreQueryFilters()` call sites** (non-test `src/backend`, semgrep scope; 369 raw text matches, 93 of them comment-only — corrected 2026-09-07, was "270" with no unit stated), **none carrying the project's own `// nosemgrep:` justification marker**, with the detecting rule running `continue-on-error`. Many are legitimate — **nobody can tell which.** | M |
...
| **GAP-020** | **~~No way to rotate a compromised JWT signing key~~ — REWORDED 2026-09-04, the original was false.** A `kid`-based key ring **exists and is tested**: `JwtKeyRingOptions.cs:21-46` + `JwtService.cs:54-72,203,228` accept overlap `ValidationKeys`, so config-change + restart rotates **without** invalidating live tokens — or with, if the compromised key is dropped, which is the compromise path. 6 real-RSA tests (`JwtServiceKeyRotationTests.cs`). **The real gap is three narrower things:** (1) **no runtime rotation** — `Program.cs:160-171` hand-constructs a singleton and snapshots `TokenValidationParameters` once, no `IOptionsMonitor` (zero production usages repo-wide), so a **rolling** restart leaves un-restarted instances honouring the compromised key with no way to observe which are clean; (2) **no global token epoch** — `ITokenDenylist` is per-`(tenant,user)`, is an `iat` cutoff a forger defeats by setting `iat = now`, fails open, and is a no-op without Redis; there is no `SecurityStamp` or token-version claim; (3) **the runbook exists only as an XML doc comment** (`JwtKeyRingOptions.cs:7-18`) — no on-call operator will find it, unlike the field-encryption key which has a README, an admin endpoint and a watchdog. **Exploitability (rated 2026-09-04, as the plan asked): attacker-exploitable = NONE/Informational (92%)** — there is no attacker-reachable primitive; the precondition is already holding the signing key, i.e. total compromise, and an admin rotate endpoint would *add* surface. **Operational recovery (MTTC) = MEDIUM (80%)** — containment is achievable but slow, undocumented, and partial during a rolling restart. **Schedule it as an ops-resilience item, not a vulnerability fix.** *(The operator-facing rotate UI is already GAP-019's §11.1 line — this row was double-counting it.)* | M (ops) |
