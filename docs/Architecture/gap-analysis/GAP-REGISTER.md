...
| ID | Gap | Size |
|---|---|---:|
| **GAP-006** | **Six tenant-scoped entities have no EF query filter** — `AuditLog`, `PayrollReportExport`, `PlanLimitOverride`, `TenantLatencyBucket`, `TenantLifecycleEvent`, `TenantScheduledJob`. RLS covers them when enabled; **`Rls:Enabled=false` in Development, so dev and CI run on one layer.** Fix + **add the coverage guard (S-2)**. | S+M |
...
| **GAP-030** | **US-ADM-012 and US-PLT-004 are `[EPIC STUB]`s** with ~50 production files and 2 migrations built against them, and **zero test cases** | M |
