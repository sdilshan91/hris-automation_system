...
| 11.9 | Performance incl. **goal cascading**, calibration, 360° | **PARTIAL** *(downgraded 2026-08-17)* | US-PRF-001..011; `GoalCommands.cs:22` (`ParentGoalId`) — **the org→department→individual cascade this row rated COVERED does not exist and will not be built.** `Goal.EmployeeId` is non-nullable (`Goal.cs:18`), so a department- or company-owned objective is not representable; `ParentGoalId` (`Goal.cs:48`) parents a goal to **another employee's goal**, never to an org tier. Tier **withdrawn** per [[ADR-2026-08-11-goal-ownership-stays-individual]]; §11.9 amended at `hrm_technical_document_v4.0.md:850`. The rating was downgraded regardless of the build/amend outcome, because it rested on backend code for a feature with no UI. Calibration and 360° carry their own gaps (GAP-021, GAP-012). |
...
- ~~**70%** — Goal cascading COVERED.~~ **RESOLVED 2026-08-17 — it flipped, exactly as predicted.** `ParentGoalId` gives goal→goal parenting; the org/dept-tier owner semantics this line called "unverified" were then verified as **absent and unrepresentable** (`Goal.EmployeeId` non-nullable). Row downgraded to **PARTIAL** at `:93`; tier withdrawn per [[ADR-2026-08-11-goal-ownership-stays-individual]]. *This is the one confidence caveat in this pass that was later measured — it was right, which is an argument for trusting the other sub-100% ratings less, not more.*
