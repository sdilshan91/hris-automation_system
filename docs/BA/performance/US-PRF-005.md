...
---
## Follow-up ACs (deferred — BUG-243, missing backend endpoints)
> Attached here rather than as net-new epics. Track in STATUS.md. Verified against `Feedback360Controller`
> (add-one reviewer POST L51, remove-one DELETE `360/reviewers/{assignmentId}` L71, results GET L126 with
> tracker embedded, feedback submit POST L106 — no full-replace, no standalone tracker, no get-form route).
- **AC-B1 — Full-replace of the manual reviewer set (FR-2).** `Feedback360Controller` offers add-one (`POST 360/cycles/{cycleId}/employees/{employeeId}/reviewers`) and remove-one (`DELETE 360/reviewers/{assignmentId}`) but **no full-replace**; the FE HR reviewer-config screen edits the whole Peer + Direct-Report set at once (`SaveReviewers`). Add `PUT 360/cycles/{cycleId}/employees/{employeeId}/reviewers`. **Input:** the complete manual reviewer set (Self + Manager are server-owned and excluded). **Behaviour:** diff against persisted assignments; re-validate BR-2 (no self-as-peer), BR-3 de-dup, and per-category minimums (FR-3); reject removing a reviewer who has already submitted → 409. **Output:** refreshed `ReviewerConfigurationDto`. **Tenant-scoping:** EF global query filter. **Authz:** `Performance.Review.All`. **Status: not built. Ref: BUG-243.**
- **AC-B2 — Standalone completion tracker (AC-3).** The FE polls a standalone per-category submitted/pending/overdue tracker (`getTracker`), but the backend only **embeds** the tracker inside the aggregated `GET 360/cycles/{cycleId}/employees/{employeeId}/results` payload. Add `GET 360/cycles/{cycleId}/employees/{employeeId}/tracker` returning just the completion tracker, OR document that the FE must derive it from `/results`. **Note:** the data already exists server-side — this is a projection/route decision, not new computation. **Tenant-scoping:** EF global query filter. **Authz:** `Performance.Review.All`. **Status: not built (data exists embedded in /results). Ref: BUG-243.**
- **AC-B3 — Fetch one reviewer's feedback form by assignment (FR-4/AC-2 deep-link).** There is **no get-form endpoint** — a reviewer can only `POST 360/cycles/{cycleId}/employees/{employeeId}/feedback` to submit. The reminder-email deep link (AC-5) needs to load the competency/goal question cards + tenant rating scale for a single assignment before submitting. Add `GET 360/assignments/{assignmentId}/form`. **Behaviour:** returns the form for the caller's own Pending assignment; the service resolves the reviewer from the session and rejects others' assignments → 403/404 (mirroring the submit endpoint's service-side ownership check). **Tenant-scoping:** EF global query filter. **Authz:** any authenticated user; ownership enforced in the service. **Status: not built. Ref: BUG-243.**
