...
- BR-7: A finalized set may be **re-opened/unlocked** by HR (`Performance.SetGoal.All`) OR by the employee's direct/finalizing manager (`Performance.SetGoal.Team`); a **mandatory audit reason** is required and re-opening resets the set back to `Acknowledged`, restoring writability. **Shipped (DF-46):** `POST /api/v1/tenant/performance/goals/reopen`.
...
- **AC-K2 — Goal re-open/unlock (DF-46).** A finalized set may be re-opened by HR (`Performance.SetGoal.All`) OR by the employee's direct/finalizing manager (`Performance.SetGoal.Team`); a **mandatory audit reason** is captured (written to the `Goal.Reopened` audit trail), and re-opening resets the set to `Acknowledged`, restoring writability. Re-opening a set that is not finalized → 409 `goals_not_finalized`; an unauthorized caller → 403 `forbidden`. **Status: SHIPPED (DF-46)** — `POST /tenant/performance/goals/reopen`; covered by TC-PRF-001-15.
