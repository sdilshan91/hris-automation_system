...
- [ ] US-PLT-004 — Observability & platform NFRs *(**⚠ PREMISES RE-VERIFIED AGAINST CODE 2026-07-30 — the stub was materially wrong; this row replaces it.** **AC-2 "health live/ready" is ALREADY BUILT** — registration `Program.cs:85-105` (Postgres readiness, Redis-as-Degraded, Redis-gated), tag-filtered mapping `:612-623` + a `/health` back-compat alias, anonymous bypass in `TenantResolutionMiddleware.cs:60-61`, and passing integration tests in `HealthCheckApiTests.cs`. **Do not rebuild it** — the only gap is that nothing calls it ([[ISSUE-346]]). **Custom `HRM.*` meters are NOT absent** — `HrmCacheMetrics.cs:16-30` ships a live `Meter("HRM.Cache")` collected by the `AddMeter("HRM.*")` wildcard; the pipeline and naming convention are proven, so the domain meters are an EXTENSION, not a bootstrap. ~~**"DORMANT" is wrong** — console-exports 100% of spans with no sampler or kill-switch~~ → ✅ **FIXED 2026-07-30 ([[ISSUE-345]]).** OTel is now **truly inert by default**: a blank endpoint with no explicit opt-in registers nothing at all. Console export requires `OpenTelemetry:Enabled=true`; `Enabled=false` is a hard kill-switch; head sampling is tunable via `OpenTelemetry:SamplingRatio` (default 1.0, clamped, never throws on a bad value). **So "dormant" is now accurate rather than aspirational** — the code was made to match what the docs already claimed, instead of correcting four documents and hoping they stayed correct. **Genuinely remaining:** Grafana LGTM stack (Phase 2, the only thing unblocking P95/latency trends) · Serilog→OTel sink · domain meters · per-tenant API-call counter · tenant span tags. **Check the GlitchTip overlap first** — US-PLT-006 already ships tenant-tagged error aggregates, so error-rate/top-errors may not need LGTM at all. Config-key trap: the plan doc says `OBSERVABILITY__OTLPENDPOINT`, the code reads `OpenTelemetry:OtlpEndpoint` ([[ISSUE-347]]).)*
