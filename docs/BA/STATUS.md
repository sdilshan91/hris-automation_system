...
> **Enterprise SSO (Microsoft Entra ID) epic — CR-AUTH-001.** PR #112 landed a **working end-to-end POC** (challenge→callback→id_token validation→fail-closed isolation→match/JIT→app JWT + FE). Reconciled status, live test results, and remaining-work checklist: **[SSO-EPIC-STATUS-AND-TODO.md](authentication/SSO-EPIC-STATUS-AND-TODO.md)**. Gated on `PlanFeatureFlags.Sso`. **EPIC COMPLETE (2026-07-28)** — 011/013/014/015 (#112) + 012 (#444) + 016 (#446). **⚠ CORRECTED 2026-08-08 (GAP-002 / [[BUG-298]]): from 2026-07-28 until 2026-08-08 the claim "the BR-5 prod gate is satisfied (DB-backed per-tenant isolation shipped)" printed here was FALSE.** #444 shipped the DB columns, the admin UI, the validators and the cache — but no login path ever read them: `EntraSsoService.CheckIsolation` still evaluated the `Authentication:Entra:TenantAllowList` **appsettings** dictionary, and the five tenant columns had zero read sites. Editing the allow-list in the UI changed nothing, admin-consent onboarding could not enable anyone, and `SsoEnabled = false` did not block SSO. **The gate is satisfied now** — the guard reads the tenant record via `SsoSettingsSnapshot`, the appsettings list is `[Obsolete]` and read by nothing, and the decision is unit-tested (`SsoIsolationGuardTests`, 17 arms) where previously 80 passing SSO tests covered none of its branches. Remaining SSO work is **test-ledger only**: the 5 `[b]` TCs in `TEST-STATUS.md` were blocked on "not implemented" and need a re-run.
...
- [x] US-AUTH-013 — Tenant-scoped tid/domain validation & isolation *(**PR #112** `CheckIsolation` + custom per-tid issuer validator, fail-closed. ⚠ **The previous claim that #444 made the allow-list DB-backed was wrong — it did not; GAP-002 did, 2026-08-08.** The guard is now the pure `SsoIsolationGuard`, evaluating the tenant's own `SsoEnabled`/`AllowedEntraTenantIds`/`AllowedEmailDomains`/`JitEnabled`/`JitDefaultRole`, with the two missing audit events `sso_isolation_rejected` and `sso_misconfigured` now emitted. **AC-7 (verified email) is closed too** (GAP-017): a domain match is honoured only when the id_token asserts `xms_edov`/`email_verified`, otherwise it falls back to the directory-id rule.)*
- [x] US-AUTH-014 — User matching, account linking & JIT provisioning *(**PR #112**: `AuthService.SsoSignInAsync` — oid→email bootstrap→JIT; JIT gated by the per-tenant `jit_enabled`/`jit_default_role` — **actually true only from GAP-002 (2026-08-08); 012 shipped the columns but the login path read appsettings**. JIT additionally now requires a VERIFIED allow-listed domain, never a directory-id match alone.)*
...
- [x] US-CHR-013 — Employee FTE & work arrangement *(**SHIPPED — CAL-6, PR #316**: `Employee.Fte` + proration (closes US-LV-002 AC-K1) · `FteScaledOvertimeBase` (default off) · `Employee.WorkArrangement` + Remote geofence exemption + ~~FE employee-form~~ **⚠ CORRECTED 2026-08-10 (GAP-L9/GAP-023): there is NO frontend for either field.** **⚠ CORRECTED 2026-08-11 (GAP-023):** the "+ FE employee-form" half of this line was **false** — zero hand-written Angular files referenced `fte` or `workArrangement`, so HR could set neither through the product (bulk import or SQL only). The backend was genuinely complete all along: entity, create command, update command with an employment-history audit entry, and a `ValidFte()` rule. **FE now shipped** — wizard (create) + profile (edit, where blank means leave-unchanged) + read-only display, with a client validator mirroring the backend rule exactly including its 2-decimal scale check.
...
- [ ] US-CHR-014 — Capture employee bank details for salary disbursement *(**NET-NEW 2026-09-07** — the expensive half of [[ENH-018]], storied out of a queue comment. `Employee.BankName`/`BankBranchCode`/`BankAccountNumber` exist (`Employee.cs:181-197`) and the masking, audit redaction and `Payroll.ViewSensitive` reveal path are all built — and all **unreachable in production, because no write path exists anywhere**; the only code that sets them is a test fixture. Needs DTO + endpoint + a NEW permission + country-aware IFSC/SWIFT validation + audit + masked-by-default reads + net-new FE capture UI. **Placed in Core HR, not Payroll:** the columns live on the tenant-isolated `employee` row and the capture surface is the employee profile (US-CHR-002) — the same call made for US-CHR-013, whose consumers were Leave and Attendance. Payroll/Reports (US-RPT-003 AC-4) are the *consumers*. ⚠ **Two hard preconditions, both stated in the story:** [[ISSUE-523]] (encrypt `bank_account_number`) must land FIRST — the no-op back-fill window closes on the first successful write — and the unmasked-`Payroll.Export` exposure must be dispositioned, since this story is what turns it from latent to live. `[ ]` but **NOT ready** — 3 open questions incl. the permission name and whether employee self-service is in scope.)*
...
- [ ] US-LV-013 — On-demand leave accrual / recalculation trigger *(**NET-NEW 2026-09-07** — the parked half of [[ENH-001]], storied out of a queue comment. The enqueue defect inside that finding (4 of 5 entitlement mutations enqueued no recalc) is **already fixed — PR #679** (`66168e95`); what remains is the manual trigger the finding is titled after. **Honest framing, per the story's §2:** this is chiefly **test-enablement + operator recovery**, not an end-user feature — US-LV-002's own test hint (`US-LV-002.md:89`, "trigger accrual job and verify ledger entries") describes a capability that does not exist, and the secondary case is forcing a run rather than waiting 24 h. No new permission (`Leave.ConfigurePolicy` matches all 12 sibling endpoints on `LeaveEntitlementsController`); no schema change expected. `[ ]` but **NOT ready** — OQ-1 (sync vs enqueued) changes AC-1 itself, and OQ-3 asks whether it is worth building at all rather than closing ENH-001 WONTFIX.)*
...
- [ ] US-REC-011 — Per-tenant interview reminder lead time *(**NET-NEW 2026-09-07** — the parked half of [[ISSUE-116]], storied out of a queue comment. The idempotency half of that finding ships separately as a T4 fix with no migration. **This closes a promise US-REC-005 BR-5 already makes** ("configurable at the tenant level") and which was never built: the lead time is the app-global key `Recruitment:InterviewReminderLeadHours` at `InterviewService.cs:474`, so every tenant on the instance shares one value. **Location DECIDED in the story:** a typed column on `Tenant` surfaced through the EXISTING `PUT /api/v1/tenant/settings/hiring` — no EAV (`TenantSettingsService.cs:19-21` forbids it), no new `RecruitmentSettings` entity, and `Tenant.AutoCreateUserOnHire` is the recruitment-scoped precedent on that same endpoint. No new endpoint or permission; the FE work is real though — there is **no hiring tab and no service method**, so that surface is net-new. **Offer-side sibling explicitly OUT of scope** (`OfferService.cs:51` `ExpiryReminderDaysBefore = 3` — a bare const, different unit and entity; its own docstring calls tenant-configurability a separate concern) — filed as a follow-up gap. `[ ]` but **NOT ready** — OQ-1 (valid range) is an acceptance criterion, not a detail.)*
...
- ~~US-PLT-002 RLS~~ — code COMPLETE, policies proven on real Postgres (Docker + native), committed **ON** *(corrected 2026-08-18 — this line said "committed OFF" and had been stale since PR #476)*; the 19 `[DEFERRED]` isolation TCs are unblocked whenever the flag is flipped.
  > **⚠ The operational consequence is the inverse of what "committed OFF" implied.** `appsettings.json:21` ships `"Rls": { "Enabled": true }`, and `DependencyInjection.cs:63-70` **refuses to start** when that is true while `ConnectionStrings:PrivilegedConnection` is blank. So a production deploy on the committed base config does not silently run without isolation — it fails loudly at startup, which is the intended fail-closed posture. Dev is held open only by `appsettings.Development.json:9`. Anyone reading "committed OFF" would have planned the flip as work still to do and been surprised by a startup failure instead. Full procedure: `docs/DEV/PRODUCTION-CHECKLIST.md` §1. Prod flip is an **ops** step, not a dev task.
...
- [~] ⛔ **US-PRF-012 — BLOCKED, DO NOT PICK UP** — `blocked: no isolation mechanism exists for a matview; see story §3 G-1..G-4` — `performance_summary` materialized view *(**NET-NEW 2026-09-07** — the parked half of [[ISSUE-129]], storied out of a queue comment **over the BA's recommendation**, precisely because a story invites someone to start. `/implement-all` must **skip this row**. **The block is CATEGORICAL, not temporal — it is NOT waiting on US-PLT-002/RLS.** PostgreSQL has **no RLS for materialized views at all**; both the policy migration (`20260710120000_Platform_RlsPolicies_Dormant.cs:55`) and the enabler (`DbInitializer.cs:218`) filter on `table_type = 'BASE TABLE'`, so the view is skipped by construction and flipping `Rls:Enabled` buys it **nothing, ever**. ⚠ **Related correction: RLS is NOT dormant** — `appsettings.json:48-50` ships `"Rls": { "Enabled": true }`; only `appsettings.Development.json` overrides it to false. **The real reason:** a matview is a physical **cross-tenant** artifact, so isolation stops being the EF global query filter and becomes a hand-written `WHERE tenant_id =` on every read — the exact class [[BUG-003]] (**CRIT, OPEN**) already exploits, and it is recorded leaking on **this same dashboard** (`TEST-FINDINGS.md:1039`, cross-tenant READ + EXPORT across overview/trend/drill-down/export). It would be the first primary user-facing read in the codebase isolated only by a hand-written predicate — and `.semgrep/tenant-isolation.yml` would **not** catch an omission, since it matches `FromSqlRaw`/`ExecuteSqlRaw` but not `FromSql`/`SqlQueryRaw`. Needs a bespoke isolation mechanism DESIGNED and agreed first (story §3 G-1..G-4); it cannot simply be unblocked later. ⚠ **The cheap half is NOT shipped either — PR #688 is absent from the merged history** (verified via `git log`; #679/#686/#691 are present). Ship the Redis read-through cache first — unblocked, tenant-prefixed keys, no isolation implications, and expected to capture most of the latency win — then re-measure. ISSUE-129 is **LOW**. Story recommends **WONTFIX on the view half** at 80% confidence.)*
