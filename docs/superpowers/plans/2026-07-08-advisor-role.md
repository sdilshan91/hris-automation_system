...
- **Report-only — advise + document.** Never edit `src/`, delete code, bump dependencies, or wire fitness tests. Writes only to `docs/Architecture/advisory-reports/`, `docs/Architecture/radar/`, and proposed ADRs in `docs/vault/decisions/`; folds actionable items into `/auto-heal` + `docs/QA/TEST-FINDINGS.md`.
...
- Create `docs/Architecture/radar/tech-radar.md` — seed radar (Adopt/Trial/Assess/Hold).
- Create `docs/Architecture/advisory-reports/.gitkeep` — output directory.
...
**Files:**
- Create: `docs/Architecture/radar/tech-radar.md`
- Create: `docs/Architecture/advisory-reports/.gitkeep`

**Interfaces:**
- Produces: the `docs/Architecture/radar/tech-radar.md` path (skill/agent update it) and the `docs/Architecture/advisory-reports/` dir (skill writes `{scope}-{date}.md` here).

- [ ] **Step 1: Create the radar seed** — write `docs/Architecture/radar/tech-radar.md`:
...
> fit-for-this-stack + migration cost. Cross-references `docs/DEV/TOOLING-ADOPTION-PLAN.md` and the ADRs.
...
Run: `mkdir -p advisory-reports && printf '# Advisory reports\n\nGenerated by /advisor (report-only). One file per run: {scope}-{YYYY-MM-DD}.md\n' > docs/Architecture/advisory-reports/.gitkeep`
Expected: `docs/Architecture/advisory-reports/.gitkeep` exists.

- [ ] **Step 3: Verify**

Run: `test -f docs/Architecture/radar/tech-radar.md && test -f docs/Architecture/advisory-reports/.gitkeep && echo OK`
Expected: `OK`

- [ ] **Step 4: Commit**

```bash
git add docs/Architecture/radar/tech-radar.md docs/Architecture/advisory-reports/.gitkeep
...
**Interfaces:**
- Consumes: `currency-scan.py` output (Task 1); `docs/Architecture/radar/tech-radar.md` + `docs/Architecture/advisory-reports/` (Task 2).
...
3. `## Passes` with three subsections:
   - **Tech-radar / currency:** run `python "$CLAUDE_PROJECT_DIR/.claude/skills/advisor/currency-scan.py" .`; read `docs/DEV/TOOLING-ADOPTION-PLAN.md`; scout via WebSearch + microsoft-learn (new tech → Assess/Hold, justify fit + migration cost).
...
- Produces: the `/advisor` user command; writes `docs/Architecture/advisory-reports/{scope}-{date}.md`, updates `docs/Architecture/radar/tech-radar.md`, proposes ADRs.
...
3. `## Process`: (a) parse flags → which passes; (b) **delegate to `@principal-advisor`** with the pass set + module scope (keep the conclusion, not raw output); (c) write `docs/Architecture/advisory-reports/{scope}-{YYYY-MM-DD}.md` from the agent's synthesis; (d) update `docs/Architecture/radar/tech-radar.md`; (e) for ADR-drift, draft proposed ADR updates in `docs/vault/decisions/` (status `proposed`) for human acceptance; (f) fold actionable items into `/auto-heal` + `docs/QA/TEST-FINDINGS.md`; (g) print a 3-line summary.
...
6. `## Graceful degradation` — if Knip/CRAP/analyzers unwired, run what's present and flag the gap (points at Wave 2/3 of `docs/DEV/TOOLING-ADOPTION-PLAN.md`).
...
Invoke `/advisor --radar` in a Claude Code session; confirm it (a) runs `currency-scan.py`, (b) updates `docs/Architecture/radar/tech-radar.md`, (c) writes an `docs/Architecture/advisory-reports/*.md` with a "Measured facts" section citing real package data, and (d) flags any unwired tools under "Gaps." No `src/` file is modified.
