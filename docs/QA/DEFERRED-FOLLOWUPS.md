...
| DF-11 | **`RejectionReason` enum columns still strict → board/detail can still 500** — PR #348 made `applicant.stage`/`.source` + `applicant_stage_history.from_stage`/`.to_stage` + `payroll_run.status` tolerant, but `applicant.rejection_reason` (`ApplicantConfiguration`) and `applicant_stage_history.rejection_reason` (`ApplicantStageHistoryConfiguration`) keep `.HasConversion<string>()`. The board materializes the **whole** Applicant entity, so a corrupt `rejection_reason` **still 500s the pipeline board** (and the detail timeline) — the board is only *partially* hardened. Needs its own `Unknown` member on the `RejectionReason` enum + a **nullable**-enum tolerant converter + a `.NotEqual(Unknown)` guard on the MoveApplicantStage `RejectionReason` rule. | [[ISSUE-231]] (PR #348) · `@integration-enforcer` | Completes the board-500 hardening; lower likelihood (only set on rejected rows, always to a validated value) but the same class. Machinery already exists — mostly mechanical. | S–M |
| DF-12 | **FE has no `Unknown` badge for a tolerated corrupt row** — `applicant.models.ts` / `pipeline.models.ts` (`STAGE_BADGE`) / `payroll-run.models.ts` (`RUN_STATUS_BADGE`/`LABELS`) are `Record<Enum,string>` with no `Unknown` key, so a row the BE now tolerates renders a **blank** badge (undefined key lookup) rather than a visible "unknown/corrupt" affordance. Graceful degradation, no crash — but silent. | [[ISSUE-231]] (PR #348) · `@integration-enforcer` | Decision: add an `Unknown` badge (surface the bad row so someone fixes the data) vs accept blank. | S |
