...
| **BUG-242 (self-assessment reopen, formerly BUG-059)** | A submitted self-assessment can't be reopened; the manager/HR reopen flow is absent. | Should reopen exist, and who can do it (HR only? within a window?)? |
...
## Ledger-hygiene TODO — DONE (2026-07-05)
The three duplicate reused IDs were de-duplicated by giving the still-open, non-PR-referenced occurrence of each a fresh ID: **ISSUE-097** vacancy-is_deleted (US-REC-001) → **ISSUE-243** (goal-audit ISSUE-097 stays, RESOLVED #154); **ISSUE-105** resume-blob-key (US-REC-003) → **ISSUE-244** (attachment-API ISSUE-105 stays, fix #177); **BUG-059** self-assessment-reopen (US-PRF-002) → **BUG-242** (Hired-terminal BUG-059 stays, fix #174). Definition headings + live `findings:` trackers in TEST-STATUS.md + the active regression TC (TC-PRF-004-16) were updated; dated 2026-06-26/27 run-log blockquotes keep the original IDs verbatim with a mapping banner at the top of TEST-FINDINGS.md.
