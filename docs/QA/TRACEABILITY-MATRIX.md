...
| US-AUTH-013 | Tenant-scoped `tid` / domain validation & isolation | Must Have | TC-AUTH-161 | 1 | 6/8 AC covered (AC-1..AC-5, AC-7 via the automated guard arms; AC-6 issuer validation is covered under US-AUTH-011, AC-8 not yet bound) |
...
| **TOTAL** | | | **169 test cases** | **169** | **95/97 AC** |
<!-- NOTE: 169 = unique auth TC files (168 prior + TC-AUTH-161, the US-AUTH-013 isolation-guard TC added 2026-09-02 to bind BUG-298/GAP-017). The AC denominator rose from 89 to 97 because US-AUTH-013 (8 ACs) entered the table with 6 covered. Previously: 168 = unique auth TC files (161 prior + 7 for US-AUTH-014: TC-AUTH-155..160 + TC-AUTH-ISO-008). The per-story TC lists above sum higher because 3 TCs serve two user stories each (listed in both rows) and TC-AUTH-ISO-008 is listed in both the US-AUTH-014 row and the cross-cutting row; the unique TOTAL is 168. -->
...
| TC-AUTH-161 | SSO isolation decided from the tenant's OWN settings — disabled/unconfigured/foreign-directory refused; a domain match needs a VERIFIED email (BUG-298/GAP-017) | Security | Critical | US-AUTH-013 | AC-1/2/3/4/5/7, FR-2, FR-5, BR-1/2/5 |
...
### US-AUTH-013 Detailed Requirements Traceability

> Bound 2026-09-02 to close the BUG-298 / GAP-017 traceability gap: the finding was being closed with **no**
> IEEE-829 test case. TC-AUTH-161 documents the 17 already-green arms of
> `HRM.Tests/Unit/SsoIsolationGuardTests` (the pure `SsoIsolationGuard.Evaluate`). It is an **automated**
> regression TC, not a manual execution record. The callback-level (real `id_token`) proof remains
> TC-AUTH-142/143, which stay `blocked` on an interactive Microsoft sign-in.

| Requirement | Type | Covered By | Coverage |
|-------------|------|------------|----------|
| AC-1: Allow-listed `tid` permitted to proceed | AC | TC-AUTH-161 | Direct (guard level) |
| AC-2: Verified email domain as the accepted alternative | AC | TC-AUTH-161 | Direct (guard level) |
| AC-3: Neither rule matches -> rejected, no JWT, no JIT, `sso_isolation_rejected` | AC | TC-AUTH-161, TC-AUTH-142 (blocked) | Partial -- the guard's refusal + reason is automated; the audit WRITE and the no-JWT response are callback-level (TC-AUTH-142) |
| AC-4: An org-B user cannot enter tenant A | AC | TC-AUTH-161, TC-AUTH-142 (blocked) | Direct at guard level; end-to-end blocked on a live IdP |
| AC-5: Enabled but empty allow-list fails closed as `sso_misconfigured` | AC | TC-AUTH-161, TC-AUTH-143 (blocked) | Direct (guard level) |
| AC-6: Issuer does not match the presented `tid` -> rejected before isolation | AC | TC-AUTH-144, TC-AUTH-145 (US-AUTH-011 issuer-validator TCs) | Indirect -- owned by the US-AUTH-011 foundation, not by this guard |
| AC-7: An UNVERIFIED email claim cannot satisfy domain allow-listing (GAP-017) | AC | TC-AUTH-161 | Direct -- `UnverifiedEmail_CannotSatisfyTheDomainRule_GAP017` |
| AC-8: The resolved tenant (from signed `state`) is the tenant used downstream | AC | -- | **Not covered** -- the guard takes the resolved tenant's settings as input; binding `state` -> tenant is callback-level (flagged) |
| FR-1 (custom issuer validator for the `organizations` authority) | FR | TC-AUTH-144, TC-AUTH-145 | Indirect (US-AUTH-011) |
| FR-2 (tenant-scoped allow-listing on the RESOLVED tenant) | FR | TC-AUTH-161 | Direct |
| FR-3 (fail closed on failure/ambiguity/empty/missing config) | FR | TC-AUTH-161 | Partial -- empty allow-list + disabled tenant automated; the **unloadable-settings** deny (`EntraSsoService.cs:222-231`) has NO test |
| FR-4 (resolved tenant comes only from the signed `state`, never the token `tid`) | FR | TC-AUTH-140, TC-AUTH-142 (blocked) | Not covered by the guard -- callback-level |
| FR-5 (only the VERIFIED email/`upn` may satisfy a domain check) | FR | TC-AUTH-161 | Partial -- the guard honours the flag; the `xms_edov`/`email_verified` claim EXTRACTION (`EntraSsoService.IsEmailVerified`, `:536-548`) has NO test |
| FR-6 (every decision audited with tenant/`tid`/domain/`oid`, raw token never logged) | FR | -- | **Not covered** -- the guard returns the reason string; `RecordSsoFailureAsync` persistence is untested (flagged) |
| FR-7 (generic non-enumerating error on rejection) | FR | TC-AUTH-142 (blocked) | Not covered at guard level -- response shaping is callback-level |
| FR-8 (this guard is the BR-5 production gate for US-AUTH-011) | FR | TC-AUTH-161 | Direct -- the DB-backed guard is what BUG-298 showed was missing |
| NFR-1 (<= 5 ms added to the callback; settings cached per tenant) | NFR | TC-AUTH-123 | Indirect -- cache behaviour tested; the 5 ms budget is not measured |
| NFR-2 (unbypassable by parameter tampering; tenant-scoped at the data layer) | NFR | TC-AUTH-ISO-005 | Indirect -- settings isolation; the state-binding half is callback-level |
| NFR-3 (issuer validator caches metadata without weakening signature validation) | NFR | TC-AUTH-144, TC-AUTH-145 | Indirect (US-AUTH-011) |
| NFR-4 (rejection timing does not leak whether a `tid`/domain is configured) | NFR | -- | **Not covered** -- needs a timing harness (flagged) |
| BR-1 (a login is valid only for the tenant whose allow-list matches) | BR | TC-AUTH-161 | Direct |
| BR-2 (`tid` primary; verified domain a permitted alternative) | BR | TC-AUTH-161 | Direct |
| BR-3 (enabled + empty allow-list rejects all logins) | BR | TC-AUTH-161, TC-AUTH-117 (config-time prevention) | Direct |
| BR-4 (the same `tid`/domain may be allow-listed by >1 tenant only deliberately) | BR | TC-AUTH-ISO-005 | Indirect -- per-tenant settings are independent; no dedicated arm |
| BR-5 (unverified email claims never trusted for an authorization decision) | BR | TC-AUTH-161 | Direct (JIT arms included) |
| BR-6 (the token `tid` never SELECTS or SWITCHES the HRM tenant) | BR | TC-AUTH-140, TC-AUTH-142 (blocked) | Not covered by the guard -- callback-level |
...
| FR-4: goal **alignment** to a parent employee goal (`parentGoalId`) | FR | -- | **WITHDRAWN as a dept/org tier — 2026-08-17.** The dept/org cascade is not deferred to a later story; it is withdrawn per [[ADR-2026-08-11-goal-ownership-stays-individual]] (a deferral implies a schedule and there is none). Goal→goal alignment ships today (`Goal.ParentGoalId`, `Goal.cs:48`). |
...
| US-ADM-011 | Approval-Workflow RUNTIME Engine (instances, routing, SLA-escalation, delegation) | Must Have | **(none authored)** — no TC doc in `docs/QA/admin-console/` binds `user_story: US-ADM-011` | 0 | **0/12 AC test-bound.** The story itself is fully authored (FR-1..13, NFR-1..5, BR-1..8) and shipped 2026-07-10 (PRs #238/#239/#240); coverage exists as **untraited** xUnit suites — `WorkflowRuntimeServiceTests`, `WorkflowRuntimeParallelPostgresTests` (AC-4), `WorkflowSlaEscalationPostgresTests` (AC-5), `WorkflowDelegationPostgresTests` (AC-6), `WorkflowRuntimeConcurrencyPostgresTests` (AC-12), `WorkflowEntityWiringPostgresTests`, `WorkflowInstanceReadPermissionAttributeTests` (AC-10), `WorkflowEvaluatorTests` (AC-3) — none of which carries a `[Trait("TC", …)]`, so none binds to a TC id. FE instance/step-chain viewer deferred → ISSUE-272. |
| US-ADM-012 | Plan / Module Governance Enforcement (runtime gating + usage limits) | Must Have | **`TC-ADM-012` (trait only — no TC doc file)** | 0 | **AC-1/AC-2/AC-4/AC-5 arms exist under `[Trait("TC","TC-ADM-012")]`** in `ModuleEntitlementMiddlewareTests`, `ModuleEntitlementApiTests`, `PlanModulesEntitlementTests`, `PlanModulesSeedDriftApiTests`, `SubscriptionPlanModuleSweepPostgresTests`, `AdminTenantPlanChangeApiTests`, `PlatformMonitoringUsageGaugesPostgresTests` — but **no IEEE 829 TC document exists for that id**, so the trait binds to nothing readable. **AC-3 is PARTIAL:** 8 of 9 limit keys block; `max_api_calls_per_month` is validated, metered and displayed but **never blocks**. |
| **TOTAL** | | | **147 test cases** | **147** | **39/39 AC** |
<!-- TOTAL stays 147: the US-ADM-011 and US-ADM-012 rows added 2026-09-04 (F4/GAP-030) each contribute ZERO test cases — neither story has a TC doc, so the TC-file count is unchanged by this edit. NOTE (reported, deliberately NOT corrected here): 147 does not reconcile with the rows above it — summing the TC Count column gives 149, and US-ADM-008/009/010 have Coverage Summaries further down but no forward row at all. Both discrepancies pre-date this change and are filed separately; fixing them here would mean inventing a number rather than recomputing one. -->
...
| TC-ADM-008-22 | Runtime role `hrm_app` cannot UPDATE/DELETE the audit tables (42501); `hrm_owner` retains DELETE for the purge (BUG-301/GAP-005) | Security | High | US-ADM-008 | AC-5, NFR-3, FR-6 (LIMIT: test hand-mirrors `roles.sql`, which no code runs) |
...
| AC-5 (audit append-only; modify/delete rejected; DB role lacks UPDATE/DELETE) | TC-ADM-008-17 (code convention) + **TC-ADM-008-22 (DB-role REVOKE, automated on real Postgres)** + TC-ADM-008-18 (DEFERRED) | Convention real; REVOKE shipped + automated (BUG-301/GAP-005), but the test hand-mirrors `roles.sql` and no code runs `roles.sql` -> live enforcement is an ops check (-18 stays deferred) |
...
| TC-ADM-009-19 | Plan-limit resolution keeps "plan_id matches no plan" (configuration error, fail closed) distinct from "plan found with NULL limit" (genuine unlimited, BR-3); override still wins; no-plans-configured is not an error; startup reconciler repoints preserving uncapped behaviour; source guard blocks the ambiguous projection | Integration | Critical | US-ADM-009 | AC-3, AC-5, FR-4, BR-3 (also US-ADM-012 AC-3) | BUG-307 (automated: PlanLimitLookupPostgresTests ×7, real PG via Testcontainers + PlanLimitLookupUsageGuardTests ×2, Docker-free) |
| TC-CHR-340 | Department list returns the manager display name and the ACTIVE employee count (batched, not N+1); no manager -> null, no employees -> 0, inactive + other-department employees excluded; list and tree render from the same flat payload | Functional | Medium | US-CHR-004 | FR-8, BR-2, §8 UI/UX columns (AC-5 display half; block is server-side) | ISSUE-364 (automated: DepartmentServiceTests ×3; FE render code-verified only) |
...
| US-PLT-004 | Observability & platform NFRs (OTel, health, per-tenant usage, SLOs) | Should Have | **(none authored)** — no TC doc binds `user_story: US-PLT-004`; `TC-PLT-004.md` binds **US-PLT-005**, read frontmatter not filenames | 0 | **0/5 AC test-bound.** AC-2 (health live/ready), AC-4 (per-tenant usage counters) and multi-tenant scoping have green but **untraited** arms (`PlatformMonitoringRedisHealthTests`, `TenantApiCallUsagePostgresTests`, `PlatformMonitoringUsageGaugesPostgresTests`, `ObservabilityExtensionsTests`, `LogCorrelationEnricherTests`). **AC-1 UNMET** (producer side only — no collector/Tempo/Prometheus in any compose file or `ops/`); **AC-3 PARTIAL** (BE computes real values; FE computes then **discards** `latencyTrend24h`/`topErrors`, `metricsStatus` hardcoded); **AC-5 UNMET** (no login-latency SLI — outcome counter only, and `/api/v1/auth` is allow-listed out of `tenant_latency_bucket`) |
...
<!-- TOTAL stays 14: the US-PLT-004 row added 2026-09-04 (F4/GAP-030) contributes ZERO test cases — the story has no TC doc at all. The count is recomputed, not inherited: 1 (US-PLT-002) + 0 (US-PLT-004) + 5 (US-PLT-005) + 8 (US-PLT-006) = 14. US-PLT-001 and US-PLT-003 remain absent from this file entirely; that hole is filed separately and was deliberately NOT widened into this change. -->
