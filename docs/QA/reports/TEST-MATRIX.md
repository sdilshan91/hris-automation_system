...
> `docs/QA/reports/` (dir + this TEST-MATRIX + the root Reports section in TRACEABILITY-MATRIX).
...
> US-RPT-005 (role-based KPI dashboard: HR / Manager / Employee) is the FINAL Reports & Analytics
> story. Implementation contract: a single server-driven endpoint `GET
> /api/v1/dashboard/widgets?refresh=` returns `{role:"hr"|"manager"|"employee", greetingName,
> generatedAt, widgets:[...]}`; `role` is DERIVED SERVER-SIDE (never trusted from the client). Each
> widget is a flexible shape `{widgetKey, label, value, previousValue, trendDirection,
> trendPercentage, trendIsPositive, miniChart{type: sparkline|donut|progress}, items[], linkUrl,
> linkFilters}`. `DashboardService` COMPOSES the existing per-module services (Core HR, Leave,
> Attendance, Recruitment, Onboarding). Redis cache TTL ~3 min keyed
> `t:{tenantId}:dashboard:{role}:{userId}:{widgetKey}`. Adds 16 test cases: 12
> functional/integration/security/performance/accessibility (TC-RPT-005-01..12) + 4 multi-tenant
> isolation (TC-RPT-ISO-017..020, continuing the running ISO counter from US-RPT-004's -016). All 5
> acceptance criteria are covered.
>
> DEFERRED / CONDITIONAL (flag to caller -- never relax a threshold to compensate):
> (1) **Module-enablement (BR-5):** there is NO per-tenant module-enablement flag in the platform
> today. TC-RPT-005-10 runs an ASSUME-ALL-ON baseline (every role-appropriate widget present); the
> disable-Payroll-and-hide steps are recorded PENDING and do NOT fail the case until a tenant
> module-flag exists.
> (2) **Redis cache (FR-4) + auto-refresh (FR-9):** Redis is deferred dev-box infra. TC-RPT-005-11 and
> TC-RPT-ISO-020 are CONDITIONAL on Redis being wired (assert the tenant+role+user key shape
> `t:{tenantId}:dashboard:{role}:{userId}:{widgetKey}`, TTL 2-5 min, and `refresh=true` bypass), else
> assert identical-on-repeat + refresh-re-queries + the intended key derivation; the NFR-1 2s / NFR-2
> 500ms thresholds are NEVER relaxed to compensate for an absent cache.
> (3) **PostgreSQL RLS (AC-5 / NFR-3):** DEFERRED defense-in-depth (consistent with the whole module).
> ISO TCs assert EF Core global query filters + TenantInterceptor + ITenantContext; cross-tenant ID
> injection asserts **404, not 403** (TC-RPT-ISO-018); the raw-SQL/RLS expectation is CONDITIONAL
> (TC-RPT-ISO-019 step 7).
> (4) **General pending-tasks/actions queue:** the Employee `pending_actions` and Manager Quick-Actions
> items depend on tasks/forms surfaces that may not all be wired; TC-RPT-005-03/-08 assert the widget
> contract + windowing/limits, treating an empty queue as a valid zero-state (TC-RPT-005-09), not a
> failure.
>
> STORY MISMATCH / SCOPE NOTES worth flagging to the caller:
> (a) Backend is forward-looking -- the unified `DashboardService` + `/api/v1/dashboard/widgets`
> endpoint is to-be-built; it composes existing per-module services. These TCs are acceptance criteria
> for the implementation.
> (b) AC-5 / NFR-3 name PostgreSQL RLS as an ACTIVE isolation layer; only the app (ITenantContext) +
> EF (query filter / TenantInterceptor) layers exist today -- RLS reworded as future hardening
> (consistent with US-RPT-001..004 and prior modules).
> (c) Manager team-scope (AC-2, TC-RPT-005-02/-07) uses the direct-reports relationship
> (`ReportsToEmployeeId`); BR-3 pending-approvals scoping is by ASSIGNED approver. The Reports module's
> `Reports.View.Team`/`.View.All` permission-granularity gap (flagged in US-RPT-001/002) does not block
> the dashboard, which derives team scope from the direct-reports data filter.
> (d) `trendIsPositive` (BR-2 / UI/UX S8) encodes BUSINESS meaning, not arithmetic sign: a rising
> headcount is positive/green; a rising turnover rate is negative/red -- TC-RPT-005-05 asserts both.

### Coverage by Test Case (US-RPT-005)

| Test Case | Title | Type | Priority | ACs / Reqs Covered | Category |
|-----------|-------|------|----------|--------------------|----------|
| TC-RPT-005-01 | HR dashboard full widget set + correct values (50 emp / 5 pending leave / 3 open pos); each widget value+trend+miniChart | E2E | Critical | AC-1, FR-1/2/3, BR-1 | Happy path |
| TC-RPT-005-02 | Manager dashboard team-scoped to 8 direct reports (ReportsToEmployeeId); pending approvals + quick actions | E2E | Critical | AC-2, FR-1/7, BR-1/3/6 | Happy / boundary |
| TC-RPT-005-03 | Employee dashboard personal widgets (leave-balance donut, attendance progress, onboarding, holidays, payslips link, pending-actions) | E2E | Critical | AC-3, FR-1/2/7, BR-1 | Happy path |
| TC-RPT-005-04 | Click-through navigates to module page with pre-applied filter via linkUrl+linkFilters (pending-leave -> /leave/requests?status=Pending) | E2E | High | AC-4, FR-5 | Happy path |
| TC-RPT-005-05 | Trend = current vs same-length prior period; trendIsPositive semantics (headcount-up=green/+; turnover-up=red/-) | Functional | High | AC-1, FR-2, BR-2 | Happy / boundary |
| TC-RPT-005-06 | Role derived server-side; role-based widget visibility; tampered ?role= ignored | Security | Critical | AC-1/2/3, FR-1/8, BR-1 | Negative / security |
| TC-RPT-005-07 | Pending Approvals counts only items assigned to the logged-in user (not all tenant-pending) | Functional | High | AC-2, FR-7, BR-3 | Happy / negative / boundary |
| TC-RPT-005-08 | Upcoming birthdays/anniversaries within next 7 days (BR-4); Quick Actions top 5 (BR-6) | Functional | High | AC-1/2, FR-6/7, BR-4/6 | Happy / boundary |
| TC-RPT-005-09 | Unauthenticated -> 401; role with no data -> empty/zero-state widgets (not error) | Security | Critical | AC-1/2/3, FR-8 | Negative / boundary / security |
| TC-RPT-005-10 | Module-enablement hides widgets (BR-5) -- DEFERRED/CONDITIONAL, assume-all-on baseline | Functional | Medium | BR-5 (deferred), AC-1/3 | Happy / boundary |
| TC-RPT-005-11 | Dashboard <=2s P95 (NFR-1); widget <500ms (NFR-2); Redis cache TTL 2-5min + refresh bypass (FR-4); auto/manual refresh (FR-9) | Performance | High | AC-1, FR-4/9, NFR-1/2/6 | Boundary / performance |
| TC-RPT-005-12 | A11y ARIA landmarks + SR metric values (NFR-5); responsive 1/2/4-col grid at 360/768/1280/1920px (NFR-4) | Accessibility | Medium | AC-1/2/3, FR-2, NFR-4/5 | Accessibility / cross-browser |
| TC-RPT-ISO-017 | Dashboards independent across tenants; A vs B show only own data; no leakage | Security | Critical | AC-5, FR-8, BR-1 | Multi-tenant isolation |
| TC-RPT-ISO-018 | No-tenant-context rejected; cross-tenant ID injection -> 404 (not 403); spoofed tenant_id ignored | Security | Critical | AC-5, FR-8, NFR-3 | Multi-tenant isolation |
| TC-RPT-ISO-019 | EF filter constrains every per-module composition path DashboardService aggregates; RLS deferred | Security | Critical | AC-5, FR-8, NFR-3 | Multi-tenant isolation |
| TC-RPT-ISO-020 | Cache keys tenant+role+user scoped (t:{tenantId}:dashboard:{role}:{userId}:{widgetKey}); no cross-tenant/user collision (Redis-conditional) | Security | High | AC-5, FR-4/8 | Multi-tenant isolation |

### Acceptance-Criteria Coverage (US-RPT-005)

| AC | Covered By |
|----|-----------|
| AC-1 (HR dashboard: headcount, open positions, pending leave, attendance rate, upcoming birthdays, recent joiners, onboarding in progress, turnover; each card value+trend+mini chart) | TC-RPT-005-01, -05, -06, -08, -09, -11, -12 |
| AC-2 (Manager dashboard: team-scoped widgets, pending approvals, team leave calendar, upcoming reviews, Quick Actions) | TC-RPT-005-02, -06, -07, -08, -09, -12 |
| AC-3 (Employee dashboard: leave-balance donut, attendance progress, onboarding progress, upcoming holidays, recent payslips link, pending actions) | TC-RPT-005-03, -06, -09, -12 |
| AC-4 (KPI widget click-through to module page with pre-applied filter) | TC-RPT-005-04 |
| AC-5 (Tenant A vs B independent; cache keys tenant+user scoped; RLS + EF filters) | TC-RPT-005-06, TC-RPT-ISO-017, -018, -019, -020 |

### FR / NFR / BR Coverage (US-RPT-005)

| Requirement | Covered By |
|-------------|-----------|
| FR-1 (role-based HR / Manager / Employee dashboards) | TC-RPT-005-01, -02, -03, -06 |
| FR-2 (each widget: value+label+trend indicator+mini visualization sparkline/donut/progress) | TC-RPT-005-01, -03, -05, -12 |
| FR-3 (widget data aggregated from multiple modules) | TC-RPT-005-01, TC-RPT-ISO-019 |
| FR-4 (Redis cache TTL 2-5min, key t:{tenantId}:dashboard:{role}:{userId}:{widgetKey}) | TC-RPT-005-11, TC-RPT-ISO-020 (Redis-conditional) |
| FR-5 (widget click-through with contextual filters pre-applied) | TC-RPT-005-04 |
| FR-6 (Announcements/Activity Feed + upcoming events) | TC-RPT-005-08 |
| FR-7 (Quick Actions panel: manager approve/reject, employee actions) | TC-RPT-005-02, -03, -07, -08 |
| FR-8 (all data scoped by tenant_id from session + role-based permissions) | TC-RPT-005-06, -09, TC-RPT-ISO-017, -018, -019, -020 |
| FR-9 (auto-refresh every 5 min + manual Refresh button) | TC-RPT-005-11 |
| NFR-1 (dashboard load <= 2s P95) | TC-RPT-005-11 |
| NFR-2 (individual widget API <= 500ms P95) | TC-RPT-005-11 |
| NFR-3 (tenant isolation; RLS deferred -> EF filters) | TC-RPT-ISO-017, -018, -019, -020 |
| NFR-4 (responsive 360px-4K; grid 1/2/4 columns) | TC-RPT-005-12 |
| NFR-5 (WCAG 2.1 AA: ARIA landmarks, contrast, SR-friendly metric values) | TC-RPT-005-12 |
| NFR-6 (Chart.js/ngx-charts lazy render visible widgets) | TC-RPT-005-11, -12 |
| BR-1 (role-based widget visibility) | TC-RPT-005-01, -02, -03, -06, TC-RPT-ISO-017 |
| BR-2 (trend = current vs same-length previous period; trendIsPositive business semantics) | TC-RPT-005-05 |
| BR-3 (Pending Approvals only items assigned to the logged-in user) | TC-RPT-005-02, -07 |
| BR-4 (Upcoming Birthdays/Anniversaries within next 7 days) | TC-RPT-005-08 |
| BR-5 (widgets respect module enablement) | TC-RPT-005-10 (DEFERRED -- no tenant module flag; assume-all-on) |
| BR-6 (Quick Actions surfaces top 5 most urgent items) | TC-RPT-005-08, -02 |

---

## Reports & Analytics -- Module Completion Summary

US-RPT-005 is the FINAL story of the Reports & Analytics module. Module coverage is now COMPLETE.

| User Story | Title | Functional TCs | ISO TCs | AC Coverage |
|-----------|-------|----------------|---------|-------------|
| US-RPT-001 | Pre-Built HR Reports (Headcount, Turnover, Demographics, ...) | TC-RPT-001-01..12 | TC-RPT-ISO-001..004 | 5/5 |
| US-RPT-002 | Leave & Attendance Reports | TC-RPT-002-01..12 | TC-RPT-ISO-005..008 | 5/5 |
| US-RPT-003 | Payroll Reports & Summaries | TC-RPT-003-01..12 | TC-RPT-ISO-009..012 | 5/5 |
| US-RPT-004 | Export Reports to CSV / PDF / Excel | TC-RPT-004-01..12 | TC-RPT-ISO-013..016 | 5/5 |
| US-RPT-005 | Dashboard with KPI Widgets | TC-RPT-005-01..12 | TC-RPT-ISO-017..020 | 5/5 |
| **Total** | **5 stories** | **60** | **20** | **25/25 AC** |

**Module totals:** 5 user stories, 80 test cases (60 functional/integration/security/performance/
accessibility + 20 multi-tenant isolation), 25/25 acceptance criteria covered.

**Module-wide deferred/conditional items (carried across stories, flagged to caller -- never relaxed):**
(1) PostgreSQL RLS (AC-5/NFR named in every story) is DEFERRED defense-in-depth; isolation enforced
via EF Core global query filters + TenantInterceptor + ITenantContext; cross-tenant ID injection
asserts 404 not 403. (2) Redis report/dashboard cache is deferred dev-box infra -- all cache TCs are
CONDITIONAL on Redis being wired; perf thresholds never relaxed to compensate. (3) The generic
cross-module reporting/dashboard backend (`/api/v1/reports`, `/api/v1/dashboard/widgets`) is
forward-looking/to-be-built -- these TCs are acceptance criteria for the implementation. (4)
`Reports.View.Team`/`.View.All` permission-granularity split needs scoped permission variants ADDED
(only a single `Reports.View` + `Reports.Export` exist today); the dashboard derives manager team scope
from the `ReportsToEmployeeId` direct-reports filter rather than a scoped permission. (5)
Module-enablement (US-RPT-005 BR-5) has no per-tenant flag yet (assume-all-on).
