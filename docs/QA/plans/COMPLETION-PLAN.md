...
| TAX-3 YTD-cumulative | P2 | persist TaxableIncome+IncomeTaxWithheld on PayrollSlip; tax(YTD)−withheldYTD keyed to country FY | TODO | — | ▶ NEXT (after TAX-2) |
| TAX-4 Year-end statement | P2 | US-PAY-009 via `PayrollReportExport` async pipeline (replaces the `deferred:true` stub) | TODO | — | after TAX-3 |
