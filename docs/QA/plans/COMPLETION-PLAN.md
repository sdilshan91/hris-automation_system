...
5. ~~**P2-2 RBAC-scope** — ISSUE-195 + BUG-120.~~ ✅ **DONE (PR #271)** — ISSUE-195: 6 HR report builders now honor BR-2 scope via the shared resolver + cache-key folds scope (+ a cache-leak isolation test); BUG-120 was already fixed on base (dup of ISSUE-018/#147) → closed as duplicate. BE 3635/3635; auditors WIRED+AUTHENTIC. Parked ENH: purpose-built `Reports.View.Team` perm (needs-decision).
6. ~~P3-3 Permission cache → Redis · P3-4 PII-at-rest~~ ✅ **DONE (#274, #273).** · **P3-1 ClamAV** (wire client; the live scan needs a ClamAV daemon → infra-gated) — NEXT.
