...
| TAX-2 Country-aware exemptions | P2 | configurable exemptions entity (per-country) → `StatutoryCalculator.ComputeTaxableIncome` (run hardcodes DeclaredExemptions:0) | TODO | — | ▶ NEXT; flexible FlatAmount/PercentOfGross/PercentOfComponent + cap + per-period |
| TAX-3 YTD-cumulative | P2 | persist TaxableIncome+IncomeTaxWithheld on PayrollSlip; tax(YTD)−withheldYTD keyed to country FY | TODO | — | after TAX-2 |
