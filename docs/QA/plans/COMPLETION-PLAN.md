...
- **[DECISION] Encryption key-rotation SOP** (P3-4) — the key-ring supports rotation (`enc:v1:{kid}`), but no rotation/retention runbook is written.
- **[DECISION] `Reports.View.Team` dedicated permission** (ISSUE-195) — report scope currently borrows the module `.View.All` perms; a purpose-built `Reports.*` scoped perm would be self-describing.
...
1. **P2-2 MED functional cluster** (real user/security bugs — the highest-value remaining band). Lead with **ISSUE-226** (offer stored-XSS — security). Then: **RBAC/team-scope** BUG-120-class residue; **audit gaps** BUG-081/083/085, ISSUE-120/200 (PII-read audit, onboarding audit); **payroll semantics** ISSUE-153/154/156/157/165/166/167/170/177/178/180 + BUG-061/062/074/079; **a11y** BUG-108/109/110/112; **UTC-boundary** BUG-245/246; **Redis** BUG-115/116; **recruitment/perf gaps** ISSUE-133/137/140/141/145/232. Fan out one sub-cluster per branch.
...
3. **P3-1 ClamAV** (security; **infra-gated**) — wire a config-gated `ClamAvVirusScanner` (+ nClam pkg) keeping the `AllowWithLogVirusScanner` fallback. Real value but the LIVE scan needs a running ClamAV daemon (ops) + can't be end-to-end-tested here without it. **Needs a go/skip decision** (package + infra commitment).
...
| P2-2 MED clusters (rest) | P2 | audit gaps; payroll semantics; a11y; UTC; Redis; ISSUE-226 XSS | TODO | — | fan-out; BUG-082 broaden-audit now unblocked |
