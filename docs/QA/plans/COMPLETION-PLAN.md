...
| P2-1c NTF Phase 8 tail | P2 | **LeaveReportExportJob:70** ("your leave export is ready" → requester) + **BulkEmployeeImportService:316** (import-completion → initiator) — 2 more inline deferred-notify sites (found by the Phase 7 integration-enforcer; Explore missed them) | TODO | — | ⚠ NOT drop-in: LeaveReportExportJob params carry NO requester userId → thread a `requestedByUserId` from the enqueue controller (has ICurrentUser); BulkImportJob has `InitiatedBy` (string — verify email vs display-name before choosing RecipientEmail vs a userId lookup). Then US-NTF-006 delivery is truly DONE. Consider reusing/adjacent to the existing `data_export_ready` event for the leave export. |
