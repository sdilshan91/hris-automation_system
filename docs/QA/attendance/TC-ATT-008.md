...
# TC-ATT-008: Clock-in is forbidden without the Attendance.CheckIn permission (authorization)

## 1. Test Objective
Verify that the clock-in endpoint enforces the `Attendance.CheckIn` permission. An authenticated user who lacks this permission must be denied with 403 Forbidden, and no `attendance_log` record may be created.

## 2. Related Requirements
- User Story: US-ATT-001
- Preconditions (permission gate): `Attendance.CheckIn`
- Functional Requirements: FR-1

## 3. Preconditions
- Tenant "acme" exists, `active`, Attendance module enabled.
- User "Riley Park" is authenticated in "acme" with a valid JWT but does NOT hold the `Attendance.CheckIn` permission (e.g., a read-only auditor role).
...
| User | Riley Park | Authenticated, lacks Attendance.CheckIn |
...
| 4 | Grant the permission and retry (positive control) | After `Attendance.CheckIn` is granted, the same request returns 201 Created. |
