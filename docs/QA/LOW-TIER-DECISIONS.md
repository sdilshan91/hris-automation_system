...
- **ISSUE-068 permission drift (Attendance)** — spec `Attendance.Clock.Self` vs shipped `Attendance.CheckIn`. · *rec: align the spec/US to the shipped `Attendance.CheckIn`.* (multi-location geofence = DF-23.)
