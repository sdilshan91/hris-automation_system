...
- **SURVEY:** **0 of 330** FE spec files (`src/frontend/src/**/*.spec.ts`) import the generated `api-types.ts` or bind an assertion to `contracts/openapi/hrm-v1.json`; 121 of 330 use `HttpTestingController` and 86 of 330 assert `req.request.body` against hand-written literals. One spec merely *mentions* the contract in a comment (`payroll-approval.service.spec.ts:28`). Total spec cases: **4,373** `it(`/`test(` calls — the entry's "4,327" is stale by 46. **Excluded and material:** `src/frontend/e2e/` holds **4** Playwright specs that drive the real stack (`fixtures/auth.ts:17`).
...
### ISSUE-545 — five live HIGH/CRIT entries are marked OPEN although their fixes are merged, and the loop keeps re-reading them as work
- **Type / Severity / Status:** ISSUE · **MED** · OPEN
- **Layer:** TEST (ledger)
- **Module / US / TC:** ledger hygiene · surfaced by the CRIT/HIGH backfill itself
- **Title:** Auditing the 18 live CRIT/HIGH findings against `src/` found that **5 of them describe defects that no longer exist**, each fixed by a PR already merged into `origin/test/local-subdomains`. Their status lines still read `OPEN`, so they inflate the backlog, distort the severity mix that drives queue ordering, and invite a second fix of an already-fixed defect.
- **SURVEY:** **5 of 18** live CRIT/HIGH entries are stale-OPEN (unit = live `OPEN`/`DEFERRED` findings at CRIT or HIGH, audited against `src/` at `8264d6d5`): `BUG-441` (CRIT — fixed by **#588**), `ISSUE-501` (**#661**), `ISSUE-036` (**#655**), `BUG-450` (**#626**), `ISSUE-486` (**#634**), plus `BUG-431` (**#581**) — **6** including `BUG-431`, which the same audit found merged. **That is one third of the slice.** The MED/LOW/unrated remainder (154 entries) has **not** been audited, so the true repo-wide count is unknown and is very likely higher.
- **AUDIT (2026-09-07):** each of the six was verified with `git merge-base --is-ancestor <fix-commit> origin/test/local-subdomains`, and in every case the defect's code claim was re-read at HEAD and found false — evidence is recorded in the `AUDIT` bullet on each entry. This finding does **not** close them: only `/verify-fix` may, and only on re-run TC evidence.
- **Severity rationale:** MED — no product defect, but it is the same class as `ISSUE-437` (nothing verifies a documented claim) applied to the ledger's own status field, and it is what makes a "181 live findings" backlog figure untrustworthy. `ISSUE-541` covers terminal-status entries sitting in the working file; this is the inverse — live-status entries whose work is done.
- **Suggested direction (NOT applied):** run `/verify-fix` on the six above; then extend the audit to the MED/LOW remainder before trusting any backlog count.
...
### ISSUE-552 — three live CRIT cross-tenant findings appear in no worklist, because the ledger's finding detector excludes "family sub-entries" by heading substring
- **Type / Severity / Status:** ISSUE · **HIGH** · OPEN
- **Layer:** TEST (tooling)
- **Module / US / TC:** ledger tooling · surfaced by the CRIT/HIGH backfill itself
- **Title:** The detector in `docs/QA/plans/SURVEY-AUDIT-BACKFILL-PROMPT.md:86-88` drops any heading matching `\b(NOTE|note|EXTENSION|EXTENDED)\b` as a sub-entry "belonging to its parent, not the worklist". Three of the entries it drops are **live CRIT cross-tenant leaks** carrying their own full `Type / Severity / Status: BUG · CRIT · OPEN` line. They are not annotations on a parent — they are findings, and no worklist built this way will ever schedule them.
- **SURVEY:** **10** headings are excluded as family sub-entries (unit = `### TYPE-NNN` definition headings matching the filter, at `8264d6d5`); **7 are live** (`OPEN`/`DEFERRED`) and **3 of those are CRIT**: `BUG-003 EXTENSION` — sign-off meeting-notes surface; `BUG-003 EXTENSION` — PIP create WRITE leaks cross-tenant (acme HR `.All` + spoofed `X-Tenant-Subdomain`); `BUG-003 EXTENSION` — Recruitment Dashboard READ re-scopes to the spoofable `X-Tenant-Subdomain`. The remaining 3 excluded CRITs are terminal (`EXTENDED to leave-entitlements` / `holiday calendar` / `carry-forward preview read`) and are correctly out of scope. Effect on the slice this backfill was scoped to: the detector reports **3 CRIT + 29 HIGH**, while the true live CRIT count is **6**. Excluded from this survey: `TEST-FINDINGS-RESOLVED.md`, which the detector never scans.
- **AUDIT (2026-09-07):** (1) The exclusion and its rationale comment — **CONFIRMED** at `plans/SURVEY-AUDIT-BACKFILL-PROMPT.md:86-88`. (2) "The three are live CRIT" — **CONFIRMED** by reading each entry's own status line; all three are `OPEN` and all three describe a spoofable-header tenant re-scope, the systemic `BUG-003` class. (3) **The filter also false-positives on standalone findings**: it is a substring test on the whole heading, so `ISSUE-121` (`:1060`, "...unlike the four rich-text **note** sections") and `ISSUE-175` (`:1516`, "...contradicting its own **note**") are dropped despite being ordinary independent findings — **CONFIRMED**, both read at those lines. Both are LOW, so the CRIT/HIGH slice is unaffected today, but the mechanism is severity-blind and would drop a CRIT worded the same way. (4) **A second, earlier revision of the same detector is in circulation** and was the one actually executed to launch this backfill: it matched `r'### (TYPE-\d+) . (.{0,70})'`, requiring a character after the ID **on the heading line**, and so silently skipped the 41-entry bare-heading block `BUG-308`…`ISSUE-418` — reporting **181 live / 1 CRIT / 21 HIGH** against the committed detector's **220 / 3 / 29**. The committed version at `:86` does not have that defect. **Not audited:** whether that earlier revision was copied into any other plan, skill or script.
- **Severity rationale:** HIGH — not a product defect, but three **CRIT cross-tenant** findings are invisible to the mechanism that decides what gets worked on, and tenant isolation is the one non-negotiable in this platform (Critical Rule #1). The cost is not a wrong number; it is three unscheduled CRITs.
- **Suggested direction (NOT applied):** decide sub-entry membership from a **field**, not the heading text — e.g. an explicit `- **Parent:** BUG-003` bullet — and have the detector count and print what it excluded, so a drop is loud rather than silent. Until then, treat any `EXTENSION` heading with its own `Type / Severity / Status` line as a first-class finding.
