...
> US-PRF-010 (Performance-Based Recommendations: Promotion, Bonus) is the TENTH and FINAL Performance story -- the talent-decision layer that turns published appraisal outcomes into promotion/bonus/increment/training recommendations, routes them through approval, and feeds approved decisions to Core HR / Payroll / Training. It adds 19 test cases: 15 functional/security/perf/a11y (TC-PRF-010-01..15) + 4 dedicated multi-tenant isolation on the new `recommendations` (+ budgets + approval chains + history) tables + downstream-integration events + caches + export artifacts (TC-PRF-ISO-037..040, continuing the running ISO counter from 036). All 5 acceptance criteria of US-PRF-010 are covered.
>
> KEY notes: happy path (TC-PRF-010-01) HR opens the recommendation workspace for a completed cycle with published ratings -> employees w/ final score/grade/tenure/comp/manager flag (from US-PRF-003) + empty recommendation fields (AC-1) -> create a Promotion (target grade + effective date) + justification -> Submit -> persisted tenant-scoped, linked to the performance record, routed to the approval workflow -> Pending Approval (AC-3, FR-1/4); auto-generation (TC-PRF-010-02) configure rule thresholds (rating >= 4.5 -> promotion, >= 3.5 -> bonus) -> apply across the cycle -> correct employees flagged with correct types as DRAFT suggestions only, HR must review/submit (AC-2/FR-2/BR-3); manual override (TC-PRF-010-03) change an auto-generated suggestion -> mandatory justification enforced client + server, blank -> 400 (FR-3); approval workflow (TC-PRF-010-04) submit -> routes through configured sequential/parallel approvers -> Approved (all) or Rejected (any reject stops the chain), non-assigned cannot act (FR-4/AC-3); budget tracking (TC-PRF-010-05) budget $100k + recommendations $110k -> SOFT warning (red bar), NOT a hard block, HR proceeds over budget with a justification, removal clears it, disabled = no enforcement (FR-8/BR-4); comparison view (TC-PRF-010-06) current vs. recommended grade/title/compensation per employee for promotion/increment/bonus, current side authoritative from Core HR (FR-5); summary (TC-PRF-010-07) total promotions + bonus pool + increment distribution by department + previous-cycle comparison, server-authoritative aggregates (AC-4); gates (TC-PRF-010-08) recommendations only after final ratings published (BR-1) + only after calibration complete if enabled (BR-2) + promotion requires target grade + effective date (BR-5), all server-side; scope/access (TC-PRF-010-09) employees see NO recommendations (403), managers see ONLY their team incl. IDOR block, HR Publish.All full, compensation visibility additionally role-gated server-side (AC-5/NFR-5); downstream integration (TC-PRF-010-10) approved promotion -> Core HR update, bonus -> Payroll one-time earning, training -> Training enrollment, fired on APPROVAL not submission, idempotent + tenant-scoped, submitted/rejected fire nothing (BR-6); encryption (TC-PRF-010-11) compensation fields encrypted at rest via pgcrypto -- asserted at the encryption seam (conditional, NFR-3); validation/sanitization (TC-PRF-010-12) type enum/custom-config + amount/percentage ranges + XSS/SQLi on justification/custom fields neutralized server-side (FR-1/S10); export (TC-PRF-010-13) PDF/Excel aggregate report MATCHES the dashboard stats, tenant-scoped + authz, branding on PDF, unsupported format -> 400 (FR-6); history/trend (TC-PRF-010-14) two cycles -> trend comparison correct + retained indefinitely (FR-7/BR-7); performance + accessibility (TC-PRF-010-15) auto-gen 5,000 employees <=10s (NFR-1) + workspace <=2.5s P95 paginated, no N+1 (NFR-4) + WCAG 2.1 AA responsive 360px-4K.
>
> Tenant isolation (NFR-2) for US-PRF-010: TC-PRF-ISO-037 cross-tenant read of recommendations/budgets/approval-chains/history/aggregates/compensation (Tenant B sees zero of Tenant A's, incl. by direct id), ISO-038 missing/invalid/mismatched tenant-context rejection + cross-tenant IDOR (view/create/override/submit/approve/auto-generate/export), ISO-039 cross-tenant write block + server-derived tenant_id (no body injection) + foreign employee/cycle/budget/approver rejected, ISO-040 tenant-scoped downstream integration events (Core HR/Payroll/Training) + approval/recommendation notifications + summary/budget caches + export artifacts.
>
> CONDITIONAL/DEFERRED for US-PRF-010 (written as conditional, not gaps): (1) same NFR-2 RLS caveat -- S7/NFR-2 name PostgreSQL RLS on recommendations; isolation is enforced via EF Core global query filters + TenantInterceptor with RLS noted as an extension point (ISO-037/039). (2) DEPENDS ON US-PRF-003 (manager recommendation flags), US-PRF-004 (completed cycle + published final ratings + calibration state, BR-1/BR-2), US-PRF-007 (dashboard the summary aligns with), and Core HR (grade/band/title/compensation) -- all assumed seeded. (3) BR-6 downstream integration (Core HR promotion update, Payroll one-time earning, Training enrollment) is asserted at the integration SEAM/event on the approval transition (TC-PRF-010-10, TC-PRF-ISO-040) -- the actual Core HR write / Payroll run / Training enrollment is owned by those modules; the seam fires on approval, not submission, and is idempotent + tenant-scoped. (4) NFR-3 pgcrypto column encryption of compensation fields is CONDITIONAL on column-level encryption being wired -- TC-PRF-010-11 asserts the encryption SEAM (`IFieldEncryptor`/pgcrypto) + cleartext-not-at-rest and documents pgcrypto-at-rest as the extension point (consistent with US-PRF-008 NFR-4). (5) FR-6 PDF/XLSX RENDERING is CONDITIONAL on the reporting library (QuestPDF / XLSX writer) being wired -- TC-PRF-010-13 asserts the export data model + dashboard-match + tenant branding + authz/tenant-scoping at the seam; full PDF visual fidelity is the extension point (consistent with US-PRF-005 FR-7 / US-PRF-006 FR-6 / US-PRF-007 FR-8). (6) FR-4 approval-workflow + recommendation notification DELIVERY (in-app + email) CONDITIONAL on the Notification System (S25) -- in-app push + email enqueue asserted, delivery conditional (TC-PRF-010-01/-04, TC-PRF-ISO-040). (7) Any recommendation summary/budget cache (S10) is asserted tenant-scoped, CONDITIONAL on a cache layer -- if computed on demand it asserts tenant-filtered queries with no shared/global key (TC-PRF-ISO-040). (8) NFR-1 5,000-employee auto-gen <=10s + NFR-4 <=2.5s P95 paginated require a seeded performance environment (TC-PRF-010-15). (9) Budget tracking is OPTIONAL/tenant-disableable; the BR-4 over-budget WARNING-not-block + proceed-with-justification path is asserted against whatever the implementation documents (TC-PRF-010-05) -- the exact soft-warning gate is an implementation contract, not a gap. (10) Custom recommendation types are tenant-configured (TC-PRF-010-02/-12) -- the per-tenant custom-type list is a config seam.

## Summary

| Metric | Value |
|--------|-------|
| Total User Stories Covered | 10 (US-PRF-001, US-PRF-002, US-PRF-003, US-PRF-004, US-PRF-005, US-PRF-006, US-PRF-007, US-PRF-008, US-PRF-009, US-PRF-010) |
| Total Test Cases | 183 (147 functional/security/perf/a11y + 36 dedicated multi-tenant isolation) |
...
| US-PRF-010 Test Cases | 19 (TC-PRF-010-01..15 + TC-PRF-ISO-037..040) |
| Critical Priority | 63 (US-PRF-001: -01/-02/-07/-08 + ISO-001/-002/-003; US-PRF-002: -01/-07/-09 + ISO-005/-006/-007; US-PRF-003: -01/-02/-07/-09 + ISO-009/-010/-011; US-PRF-004: -01/-02/-04 + ISO-013/-014/-015; US-PRF-005: -01/-05/-06 + ISO-017/-018/-019; US-PRF-006: -01/-07/-11 + ISO-021/-022/-023; US-PRF-007: -01/-06/-08 + ISO-025/-026/-027; US-PRF-008: -01/-07/-09/-10 + ISO-029/-030/-031; US-PRF-009: -01/-08/-10/-13 + ISO-033/-034/-035; US-PRF-010: -01/-09 + ISO-037/-038/-039) |
| High Priority | 119 (remaining functional/perf/a11y of all ten stories + TC-PRF-ISO-004, -008, -012, -016, -020, -024, -028, -032, -036, -040) |
| Medium Priority | 1 (TC-PRF-010-13) |
| Low Priority | 0 |
| Blocked Test Cases | 0 |
| Acceptance Criteria Coverage | US-PRF-001 5/5; US-PRF-002 5/5; US-PRF-003 5/5; US-PRF-004 5/5; US-PRF-005 5/5; US-PRF-006 4/4 (US-PRF-006 has only AC-1..AC-4); US-PRF-007 5/5; US-PRF-008 5/5; US-PRF-009 5/5; US-PRF-010 5/5 |
| Status | All Draft |

> Note: Critical-priority IDs total 63 across the ten stories (US-PRF-001..009 = 56 as before; US-PRF-010: -01/-09 + ISO-037/-038/-039 = 5; -01 happy-path E2E + -09 scope/access security are Critical, -11/-12 are High security, ISO-040 is High); High totals 119; Medium 1 (US-PRF-010-13 export); summing to 183.
...
| US-PRF-010 | Performance-Based Recommendations (Promotion, Bonus) | TC-PRF-010-01, TC-PRF-010-02, TC-PRF-010-03, TC-PRF-010-04, TC-PRF-010-05, TC-PRF-010-06, TC-PRF-010-07, TC-PRF-010-08, TC-PRF-010-09, TC-PRF-010-10, TC-PRF-010-11, TC-PRF-010-12, TC-PRF-010-13, TC-PRF-010-14, TC-PRF-010-15 | 15 |
| Cross-cutting (PRF-010) | Multi-tenant isolation (recommendations + budgets + approval chains + history tables + downstream-integration events + caches + export artifacts) | TC-PRF-ISO-037, TC-PRF-ISO-038, TC-PRF-ISO-039, TC-PRF-ISO-040 | 4 |
...
| FR-4 goal **alignment** (`parentGoalId`, goal→goal) | **Dept/org tier WITHDRAWN 2026-08-17** ([[ADR-2026-08-11-goal-ownership-stays-individual]]) — not deferred. Goal→goal alignment is shipped and testable. |
