# .NET Common Anti-patterns (reference)

> **Provenance:** vendored from [`codewithmukesh/dotnet-claude-kit`](https://github.com/codewithmukesh/dotnet-claude-kit)
> (`knowledge/common-antipatterns.md`), MIT © 2025 Mukesh Murugan. Lightly retargeted to this repo.
> A BAD/GOOD C# reference for `@backend-dev` and reviewers. The `antipattern-advisor` PreToolUse hook
> greps for the four most mechanically-detectable of these on every `.cs` Write/Edit (advisory only).
> Deeper C#/EF standards live in the vendored `dotnet-skills` plugin (`csharp-coding-standards`,
> `efcore-patterns`); this file is the quick, self-contained BAD/GOOD cheat-sheet.
...
(In this repo, outbound calls should use the named `ResilientClient` HttpClient with Polly retry +
circuit-breaker — see `CLAUDE.md` › Cross-cutting backend infrastructure.)
...
(This repo already relies on a clock seam for date-dependent jobs — inject `TimeProvider` and use
`FakeTimeProvider` in tests. See the CAL-8 "date-dependent jobs need a clock seam" lesson.)
...
(In this repo, let unexpected exceptions propagate to `ExceptionHandlingMiddleware`, the outermost
layer that normalizes errors.)
...
(Also a PII risk — see the security-audit rule "log identifiers, not identity data".)
...
(This repo's convention is NoTracking-by-default — see `efcore-patterns`.)
