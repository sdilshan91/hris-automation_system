...
description: Pick the next pending user story from docs/BA/STATUS.md and build it end-to-end (BE + FE + QA on one feature branch + PR). One story per invocation.
---
...
- **Empty** → pick the first `[ ]` story in [docs/BA/STATUS.md](docs/BA/STATUS.md) across all modules in priority order.
...
1. Read [docs/BA/STATUS.md](docs/BA/STATUS.md), pick the target story
...
5. Launch **three sub-agents in parallel in a single message**:
   ...
   - `@qa-engineer` → `docs/QA/{module}/` only (NO git commands)
