...
# Detects docs/BA/ changes and triggers dev + QA agents in parallel
...
if echo "$CHANGED_FILES" | grep -q "^docs/BA/"; then
    echo "========================================="
    echo " USER STORIES COMMITTED - TRIGGERING AGENTS"
    echo "========================================="

    # Extract which modules had story changes
    MODULES=$(echo "$CHANGED_FILES" | grep "^docs/BA/" | sed 's|docs/BA/||' | cut -d'/' -f1 | sort -u)
    ...
    for MODULE in $MODULES; do
        ...
        STORY_FILES=$(echo "$CHANGED_FILES" | grep "^docs/BA/$MODULE/")
        ...
        echo "  [QA-ENGINEER]  Ready to write test cases in: docs/QA/$MODULE/"
        echo ""
    done
