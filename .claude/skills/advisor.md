...
2. **Delegate to `@principal-advisor`** via the Agent tool, passing the resolved pass set and
   ...
   re-derive findings.
3. **Write `advisory-reports/{scope}-{YYYY-MM-DD}.md`** from the agent's synthesis, preserving its
   ...
   and gaps. Create the `advisory-reports/` directory if absent. This file is the durable evidence
   trail — do not summarize it away.
4. **Update `docs/radar/tech-radar.md`** from the agent's "Tech-radar deltas" section: for each
...
6. **Fold actionable items into `/auto-heal` + `test-cases/TEST-FINDINGS.md`.** Any
...
7. **Print a 3-line summary**: the verdict line, the count of recommendations by severity (e.g.
   "3 findings: 1 HIGH, 2 MED"), and the paths written (`advisory-reports/...`, `docs/radar/
   tech-radar.md`, and any proposed ADR files).

## Report-only boundary

`/advisor` **writes only** to `advisory-reports/`, `docs/radar/tech-radar.md`, and *proposed*
...
- **`/security-audit`** — diff-scoped vulnerability review; `/advisor` cites its existing
  `security-reviews/*.md` output where a finding overlaps, never re-runs it.
- **`/design-review`** — visual/UX audit; same linkage via `design-reports/*.md`.
...
`test-cases/TEST-FINDINGS.md` and re-sorts the living completion plan — `/advisor` is an input to
...
`docs/TOOLING-ADOPTION-PLAN.md` (Wave 2/3 covers Knip, CRAP analysis, and the remaining static
