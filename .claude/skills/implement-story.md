...
- `/implement-all` = picks the next pending story from `docs/BA/STATUS.md`, then runs essentially this same flow, and flips the status afterward.

Both produce **one branch + one commit + one PR per story** with FE + BE + QA bundled.

## Process

1. **Validate input** — story ID matches `US-[A-Z]+-\d{3}` and the story file exists at `docs/BA/{module}/US-{ID}.md`.
...
Unlike `/implement-all`, this skill leaves `docs/BA/STATUS.md` alone. If you want the tracker updated, hand-edit STATUS.md or use `/implement-all` instead.
