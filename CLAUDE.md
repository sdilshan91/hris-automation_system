...
| `/plan-audit [--living\|--module\|--drift-only]` | Local | **Cross-doc plan auditor (REPORT-ONLY).** Scans every plan/status/ledger doc, computes real %-complete + remaining points, and cross-checks each "done" claim against **ground truth** (merged PR#s, ledger `RESOLVED`, TC `status:`) to surface **drift** — stale checkboxes (merged-not-closed), status conflicts, duplicate tracking of one item across docs, unverified claims, and unregistered plan docs — into a single regenerated `test-cases/PLAN-AUDIT.md`. Hybrid: deterministic `scan.py` + a reconciler sub-agent. Never edits a source plan (the **read-side** counterpart to `/auto-heal`; feeds it). |
...
│   │   ├── plan-audit.md          # Cross-doc plan auditor (report-only); scan.py + sub-agent
