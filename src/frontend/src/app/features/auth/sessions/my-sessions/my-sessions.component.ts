...
/**
 * US-AUTH-009 AC-6: User's own active sessions view.
 * Displays session cards with device/browser/OS, IP, timestamps,
 * "Current session" badge, and Revoke button.
 * The current session's Revoke is disabled (BR-4).
 */
@Component({
      ...
      <div class="sessions-card">
        <div class="sessions-header">
          ...
          <p class="sessions-subtitle">
            ...
            except the one you are currently using.
          </p>
        </div>
            ...
            @for (session of sessions(); track session.sessionId) {
              ...
              >
                ...
                <div class="session-actions">
                  <button
                    class="btn-revoke"
                    [disabled]="session.isCurrent || revokingId() === session.sessionId"
                    [title]="session.isCurrent ? 'Cannot revoke current session. Use logout instead.' : 'Revoke this session'"
...
export class MySessionsComponent implements OnInit {
  ...
  loadSessions(): void {
    ...
    this.authService.getMySessions().subscribe({
      ...
      error: (err: HttpErrorResponse) => {
        this.isLoading.set(false);
        this.loadError.set(
          err.error?.message || 'Failed to load active sessions.'
        );
      },
    });
  }
  ...
  revokeSession(session: ISession): void {
    if (session.isCurrent || this.revokingId()) {
      return;
    }
    ...
    this.authService.revokeSession(session.sessionId).subscribe({
      ...
      error: (err: HttpErrorResponse) => {
        this.revokingId.set(null);
        this.toastr.error(
          err.error?.message || 'Failed to revoke session. Please try again.'
        );
      },
    });
  }
