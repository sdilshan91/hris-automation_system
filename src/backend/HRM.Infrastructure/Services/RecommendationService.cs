...
public sealed class RecommendationService : IRecommendationService
{
    ...
    public async Task<Result<AutoGenerateResultDto>> AutoGenerateAsync(Guid cycleId, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var review in reviews)
        {
            ...
            // ISSUE-149a: an auto-generated suggestion is still a CREATE — same action and same payload shape
            // as the manual path, with isAutoGenerated distinguishing it. Staged; committed with the batch below.
            _auditLogger.Log(
