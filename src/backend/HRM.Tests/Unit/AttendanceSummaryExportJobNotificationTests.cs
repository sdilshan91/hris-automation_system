// ============================================================================
// US-NTF-006 (P2-1d) — AttendanceSummaryExportJob.DispatchReportReadyAsync unit tests.
//
// Symmetric counterpart of LeaveReportExportJobNotificationTests for the attendance-export side: the job sends
// an `attendance_summary_export_ready` notification (in-app + email) to the REQUESTER (requestedByUserId),
// carrying the report period (YYYY-MM) + download locator. Guarded so a delivery failure never aborts the
// export; skipped when no requester id. Uses a hand RecordingDispatcher whose captured requests are asserted.
// ============================================================================
...
public sealed class AttendanceSummaryExportJobNotificationTests
{
    ...
    public async Task Dispatch_SendsBothLegsToRequester_WithPopulatedPayload()
    {
        ...
        await AttendanceSummaryExportJob.DispatchReportReadyAsync(
            dispatcher, Tenant, Requester, 2026, 6,
            "https://app.example.com/exports/attendance/abc/download?token=x");
        ...
        req.EventKey.Should().Be("attendance_summary_export_ready");

        // Payload-completeness: the template renders report.type (YYYY-MM) + report.downloadUrl — prove both supplied.
        using var doc = JsonDocument.Parse(req.PayloadJson);
        var report = doc.RootElement.GetProperty("report");
        report.GetProperty("type").GetString().Should().Be("2026-06");
        report.GetProperty("downloadUrl").GetString()
            .Should().Be("https://app.example.com/exports/attendance/abc/download?token=x");
    }
    ...
    public async Task Dispatch_Skipped_WhenNoRequesterId()
    {
        ...
        await AttendanceSummaryExportJob.DispatchReportReadyAsync(
            dispatcher, Tenant, Guid.Empty, 2026, 6, "https://x/y");
    ...
    public async Task Dispatch_NeverThrows_WhenDispatcherFails()
    {
        ...
        var act = () => AttendanceSummaryExportJob.DispatchReportReadyAsync(
            dispatcher, Tenant, Requester, 2026, 6, "https://x/y");
