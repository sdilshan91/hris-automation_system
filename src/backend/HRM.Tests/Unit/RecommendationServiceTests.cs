...
public sealed class RecommendationServiceTests
{
    ...
    private RecommendationService Service(ICurrentUser user) => Service(user, Db());

    /// <summary>Overload for tests that must inspect the SAME DbContext instance the service mutated (ENH-015).</summary>
    private RecommendationService Service(ICurrentUser user, AppDbContext db)
        => new(db, _tenantContext, user,
    ...
    // ── ENH-015: dry-run preview of auto-generation ─────────────────────

    /// <summary>
    /// ENH-015: a preview must compute the SAME decisions as a real run and persist nothing. Asserted by
    /// re-querying the store on a fresh context — the returned DTO is the thing under suspicion, so trusting it
    /// would prove nothing.
    /// </summary>
    [Fact]
    public async Task AutoGenerate_dryRun_returns_the_same_suggestions_but_writes_nothing_ENH015()
    {
        await SeedAsync();
        await SeedRulesAsync();

        var preview = await Service(HrUser()).AutoGenerateAsync(_cycleId, dryRun: true);

        preview.IsSuccess.Should().BeTrue();
        preview.Value!.DryRun.Should().BeTrue();

        // NOTHING was written — the point of the feature.
        using (var db = Db())
        {
            db.Recommendations.IgnoreQueryFilters().Count(r => r.CycleId == _cycleId).Should().Be(0);
            db.RecommendationEvents.IgnoreQueryFilters().Count().Should().Be(0);
        }

        // A real run afterwards produces the SAME decisions the preview promised.
        var real = await Service(HrUser()).AutoGenerateAsync(_cycleId, dryRun: false);
        real.IsSuccess.Should().BeTrue();
        real.Value!.DryRun.Should().BeFalse();

        preview.Value.EmployeesEvaluated.Should().Be(real.Value.EmployeesEvaluated);
        preview.Value.SuggestionsCreated.Should().Be(real.Value.SuggestionsCreated);
        preview.Value.SuggestionsSkipped.Should().Be(real.Value.SuggestionsSkipped);
        preview.Value.Suggestions
            .Select(x => (x.EmployeeId, x.Type, x.Status, x.IsAutoGenerated, x.BonusPercent, x.IncrementPercent, x.AutoGenerationRationale))
            .Should().BeEquivalentTo(real.Value.Suggestions
                .Select(x => (x.EmployeeId, x.Type, x.Status, x.IsAutoGenerated, x.BonusPercent, x.IncrementPercent, x.AutoGenerationRationale)));
    }

    /// <summary>
    /// ENH-015, the AppendEvent trap. AppendEvent runs while the new Recommendation is still Detached, so its
    /// "not Added yet" branch calls <c>RecommendationEvents.Add(ev)</c> — putting an Added row into the change
    /// tracker even though the recommendation itself is never added. A preview that called it would leave the
    /// DbContext dirty and the NEXT unrelated SaveChanges in the same scope would commit the preview.
    /// This arm reproduces that scope: one context, a dry run, then an unrelated save.
    /// </summary>
    [Fact]
    public async Task AutoGenerate_dryRun_leaves_the_context_clean_so_a_later_unrelated_save_commits_nothing_ENH015()
    {
        await SeedAsync();
        await SeedRulesAsync();

        using var scoped = Db();
        var result = await Service(HrUser(), scoped).AutoGenerateAsync(_cycleId, dryRun: true);
        result.IsSuccess.Should().BeTrue();
        result.Value!.SuggestionsCreated.Should().BeGreaterThan(0); // the preview really did decide something

        // The change tracker must hold nothing from the preview — neither the recommendations nor their events.
        scoped.ChangeTracker.Entries<Recommendation>().Should().BeEmpty();
        scoped.ChangeTracker.Entries<RecommendationEvent>().Should().BeEmpty();

        // An unrelated write in the SAME scope. If the preview had dirtied the context, this commits it.
        scoped.RecommendationBudgets.Add(new RecommendationBudget
        {
            Id = Guid.NewGuid(), TenantId = _tenantId, CycleId = _cycleId,
            Name = "Unrelated", AllocatedAmount = 1000m, Currency = "USD",
        });
        await scoped.SaveChangesAsync();

        using var verify = Db();
        verify.Recommendations.IgnoreQueryFilters().Count(r => r.CycleId == _cycleId).Should().Be(0);
        verify.RecommendationEvents.IgnoreQueryFilters().Count().Should().Be(0);
    }

    /// <summary>
    /// ENH-015: a preview must not FLUSH either. This is a distinct failure from the AppendEvent trap: even with
    /// nothing of its own to write, an unguarded <c>SaveChangesAsync</c> inside a "preview" commits whatever the
    /// CALLER had already staged in the same scope — turning a read-only-looking call into someone else's commit.
    ///
    /// <para>Written after the guard-removal mutant SURVIVED the arms above: with nothing tracked, a stray
    /// SaveChanges is a silent no-op, so only pre-staged caller work can observe it. That is the observable
    /// difference, so that is what this asserts.</para>
    /// </summary>
    [Fact]
    public async Task AutoGenerate_dryRun_does_not_flush_the_callers_pending_changes_ENH015()
    {
        await SeedAsync();
        await SeedRulesAsync();
        ...
        // The caller stages unrelated work and has NOT saved it yet.
        scoped.RecommendationBudgets.Add(new RecommendationBudget
        {
            Id = stagedId, TenantId = _tenantId, CycleId = _cycleId,
            Name = "Staged, not yet committed", AllocatedAmount = 500m, Currency = "USD",
        });

        var preview = await Service(HrUser(), scoped).AutoGenerateAsync(_cycleId, dryRun: true);
        preview.IsSuccess.Should().BeTrue();
        preview.Value!.SuggestionsCreated.Should().BeGreaterThan(0);

        // The preview must not have committed the caller's staged row on their behalf.
        using var verify = Db();
        verify.RecommendationBudgets.IgnoreQueryFilters().Any(b => b.Id == stagedId).Should().BeFalse();
        verify.Recommendations.IgnoreQueryFilters().Count(r => r.CycleId == _cycleId).Should().Be(0);
    }

    /// <summary>ENH-015: the default and the explicit false still persist exactly as before — no behaviour change.</summary>
    [Fact]
    public async Task AutoGenerate_dryRun_false_still_persists_ENH015()
    {
        await SeedAsync();
        await SeedRulesAsync();

        var result = await Service(HrUser()).AutoGenerateAsync(_cycleId, dryRun: false);
        ...
        using var db = Db();
        db.Recommendations.IgnoreQueryFilters().Count(r => r.CycleId == _cycleId).Should().Be(3);
        // The Created audit event is written by the act of persisting — one per suggestion.
        db.RecommendationEvents.IgnoreQueryFilters()
            .Count(e => e.EventType == RecommendationEventType.Created).Should().Be(3);
    }
