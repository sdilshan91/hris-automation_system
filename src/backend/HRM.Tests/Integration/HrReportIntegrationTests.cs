// ============================================================================
// US-RPT-001: Pre-built HR reports — integration tests.
//
// Exercises HrReportService over a real AppDbContext (InMemory) with the ITenantContext-driven global
...
//
// PROVIDER: InMemory — same rationale as the other integration tests in this project (the verify gate runs
// `dotnet test` with no PostgreSQL / Docker). The service is written to be InMemory-safe (no projection
// through a filtered required navigation; List.Contains for IN filters). No IDistributedCache is wired
// here, so each call computes fresh — caching itself is an optional Redis seam validated in prod.
// ============================================================================
...
public sealed class HrReportIntegrationTests
{
