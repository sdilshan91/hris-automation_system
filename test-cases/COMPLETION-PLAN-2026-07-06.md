...
- **▶ REFRESHED PRIORITIES (2026-07-09) — what's left, re-sorted by value × readiness × effort:**
  ...
  - **P3 — infra (in progress):**
    - ✅ **OTel + health probes** → **PR #229** (open): traces+metrics (OTLP endpoint-gated) + `/health/live|ready`.
    - ✅ **Redis query cache (foundation)** → **PR #229** (open): transparent tenant-safe EF second-level cache (`EFCoreSecondLevelCacheInterceptor`) over reference tables; Redis-or-memory; read + auto-evict-on-`SaveChanges` (NOT write-through); validated on Docker Redis. `IPermissionCache` left in-memory by decision.
    - 🟡 **TODO — Redis for ALL heavy modules / services (DIRECTIVE):** the query cache is a global interceptor — broaden its whitelist to **all read-heavy / reference / config / catalog / template tables** across every module (approved 2026-07-10: current 8 + ~20 → attendance/leave policy, catalogs/templates, plan/statutory, workflow definitions; EXCLUDE authz/identity/secrets, high-write transactional, pay-affecting, `tenants`/`employees`). Plus **targeted volatile-but-hot caches** (leave-balance/dashboard — `TODO(redis-balance-cache)`; deferred to AFTER RLS) with explicit invalidation. `IDistributedCache` (Auth/Role/UserMgmt/TenantSettings/Lifecycle/NotifPref/Dashboard/HrReport/Monitoring + TenantResolution) and the SignalR backplane are already Redis-when-configured. **Provisioning:** prod needs a real Redis (dev/tests blank `ConnectionStrings:Redis` → in-memory fallback; never a hard dependency).
    - 🔴 **TODO — RLS FULL-ENABLE for ALL services (DIRECTIVE — SaaS security best practice; decision MADE 2026-07-10 = full enable, NOT just go/no-go):** author `CREATE POLICY` + `ENABLE ROW LEVEL SECURITY` migrations for **every** tenant-scoped table (0 exist today), set the `app.current_tenant` GUC per request (`TenantTransactionBehavior` — flip `Rls:Enabled=true`), route **all** untenanted paths onto the privileged BYPASSRLS connection (40+ Hangfire jobs, `DbInitializer` seeding, the tenant-resolution lookup, every `IgnoreQueryFilters`/raw-SQL path), add the `TenantTransactionBehavior:57` Postgres test + a CI postgres service-container job. **⚠ HARD PREREQ:** fix the EF-cache **`nohttp:` prefix** (background/startup queries) via an AsyncLocal ambient tenant set by both `TenantResolutionMiddleware` and Hangfire job restoration — else RLS makes background-job cached queries leak cross-tenant. Own focused branch/PR; do AFTER #229 merges. (US-PLT-002; Theme A.)
...
> **DIRECTIVE (2026-07-10): Redis caching for all heavy modules + RLS for all services are BOTH required** (SaaS security & best practice). Tracked as standing TODOs below, not one-off tasks.

1. **🟡 Redis — cache all heavy/read-dominated modules (DIRECTIVE):**
   - a. **Foundation DONE (PR #229):** transparent tenant-safe EF second-level query cache (reference tables) + `IDistributedCache`/SignalR already Redis-when-configured. Read + auto-evict-on-`SaveChanges` (not write-through); shared-Redis ⇒ cross-instance invalidation.
   - b. **TODO — broaden the query-cache whitelist to every read-heavy/reference/config/catalog/template table** across all modules (approved list: current 8 + ~20; exclude authz/identity/secrets, high-write transactional, pay-affecting, `tenants`/`employees`). It's a global interceptor, so this uniformly benefits every service.
   - c. **TODO — targeted volatile-but-hot caches** with explicit invalidation (deferred to AFTER RLS): leave dashboard/balance (`TODO(redis-balance-cache)` → resolves **ISSUE-039** N+1), holiday (now covered by b), **ENH-017** statutory-rule TTL, and the Phase-1 session-revocation denylist. `IPermissionCache` stays in-memory (decision) unless revisited.
   - d. **Provisioning:** prod needs a real Redis instance; dev/tests keep `ConnectionStrings:Redis` blank → in-memory fallback (Redis is never a hard dependency).
2. **🔴 RLS — enable for ALL tenant-scoped services/tables (DIRECTIVE; decision = FULL enable):** US-PLT-002 / Theme A. `CREATE POLICY` + `ENABLE ROW LEVEL SECURITY` on **every** tenant table (0 today), GUC per request (`TenantTransactionBehavior`, flip `Rls:Enabled`), privileged BYPASSRLS connection for **all** untenanted paths (40+ jobs, `DbInitializer`, tenant-resolution lookup, `IgnoreQueryFilters`/raw-SQL), `TenantTransactionBehavior:57` Postgres test + CI postgres service-container. **HARD PREREQ:** fix the EF-cache `nohttp:` background-job prefix (AsyncLocal ambient tenant) or RLS leaks cached background-job queries cross-tenant. + audit-log immutability (`Persistence/Rls/README.md`, `AuditLogService.cs:22`, `ImpersonationService.cs:21`).
