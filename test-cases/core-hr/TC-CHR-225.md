...
> **Execution 2026-07-01 (API, acme, tenantadmin):** BLOCKED — write not cleanly reversible + needs time-travel. Code path VERIFIED by read: `EmployeeStatusService.ChangeStatusAsync` computes `isFutureDated = EffectiveDate.Date > UtcNow.Date` (line 89); when true it stores a `FutureDatedStatusChange` row (From/To/EffectiveDate/pending) and returns `IsFutureDated:true` with message "Status change to X scheduled for … applied automatically on the effective date" — the employee's live status is **NOT** changed today (steps 1-4 ✓ by inspection). A daily background job later applies pending changes whose `EffectiveDate <= today` (`GetPendingChanges` filters `!IsApplied && !IsCancelled && EffectiveDate <= today`). NOT executed as a live write because: (a) there is **no API to list or cancel** a scheduled `FutureDatedStatusChange`, so a test POST would leave an un-revertable pending mutation that the job WILL apply to a real employee on the effective date; (b) proving "applied on effective date" (steps 5-8) requires advancing the clock / triggering the Hangfire job, not available report-only. Blocked: unrevertable-write + time-travel-required (behaviour correct on read).
