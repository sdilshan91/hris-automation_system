...
**Scope of this document:** Formal write-up of 5 confirmed bugs, validated against the existing IEEE-829 test cases under `test-cases/`. **No TCs, source, or PRs were modified.** All cited code paths were re-verified against the working tree during this analysis.
...
| **BUG-2** (no sidebar entry for Tenants/Monitoring) | `TC-ADM-002-09` is the closest (mentions route guard + monitoring) but tests **deny by role**, not **nav presence**. No `navItems`-rendering TC found in `test-cases/admin-console/`. | **No.** (a) effectively no covering TC + (c) the nearest is an API/deny test. | No TC asserts the platform persona's sidebar contains reachable Tenants/Monitoring entries. Add a layout/navigation TC for the SystemAdmin persona. |
